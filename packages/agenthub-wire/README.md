...
Canonical wire specification package for AgentHub clients and services.
...
`@artsum/agenthub-wire` centralizes definitions for:
...
- Name: `@artsum/agenthub-wire`
- Workspace path: `packages/agenthub-wire`
- Entry: `src/index.ts`
...
In this repository, consumer workspaces import `@artsum/agenthub-wire` through package exports that point at `dist/*`.

That means on a clean checkout:
1. Build wire first: `npx -y pnpm@10.11.0 --filter @artsum/agenthub-wire build`
...
npx -y pnpm@10.11.0 --filter @artsum/agenthub-wire build
npx -y pnpm@10.11.0 --filter @artsum/agenthub-wire test
```
...
```bash
# from repository root
pnpm release -- @artsum/agenthub-wire
```

Prefer the repository release workflow from the root so versioning, tags, artifacts and smoke checks stay grouped with the rest of the monorepo release process.
