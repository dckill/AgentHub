...
describe('shared wire message schemas', () => {
  ...
  it('preserves extensible message mode, file references, and image metadata', () => {
    const parsed = MessageContentSchema.parse({
      role: 'user',
      content: { type: 'text', text: 'inspect these files' },
      meta: {
        permissionMode: 'team-custom-mode',
        fileReferences: ['/repo/a.ts'],
        images: [{ data: 'base64', mimeType: 'image/png', name: 'shot.png', width: 10, height: 20 }],
      },
    });
    ...
    expect(parsed.meta).toMatchObject({
      permissionMode: 'team-custom-mode',
      fileReferences: ['/repo/a.ts'],
      images: [{ mimeType: 'image/png', name: 'shot.png', width: 10, height: 20 }],
    });
  });
});
