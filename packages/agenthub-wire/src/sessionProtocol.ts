/**
 * Session protocol envelope used by the current CLI/App message pipeline.
 *
 * Encrypted session messages can still contain the legacy role-based payloads,
 * but Claude Code and Codex runners now also emit these envelopes for turn
 * lifecycle, text, tool, file and stop events. Keep this schema in sync with
 * CLI mappers and the App reducer when adding events.
 */
