...
// Resolve agenthub home dir — same pattern as agenthub-cli/src/configuration.ts
let agenthubHome: string;
if (process.env.AGENTHUB_HOME_DIR) {
    agenthubHome = process.env.AGENTHUB_HOME_DIR.replace(/^~/, homedir());
} else {
    agenthubHome = join(homedir(), '.agenthub');
}

const logsDir = join(agenthubHome, 'app-logs');
