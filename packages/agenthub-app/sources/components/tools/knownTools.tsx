    ...
    minimal: (opts: { metadata: Metadata | null, tool: ToolCall, messages?: Message[] }) => {
        ...
        for (let m of messages) {
            if (m.kind === 'tool-call'
                ...
                const childTool = knownTools[m.tool.name as keyof typeof knownTools] as any;
                if (childTool?.hidden) {
                    continue;
                }
                return false;
...
export const knownTools = {
    ...
    'Read': {
        title: (opts: { metadata: Metadata | null, tool: ToolCall }) => {
            ...
            // Historical payloads may use a 'locations' array with a path field.
        ...
        input: z.object({
            ...
            // Legacy read-only payload format.
    ...
    // Historical payloads may use lowercase 'read'.
    'read': {
        title: (opts: { metadata: Metadata | null, tool: ToolCall }) => {
            // Historical payloads may use a 'locations' array with a path field.
            if (opts.tool.input.locations && Array.isArray(opts.tool.input.locations) && opts.tool.input.locations[0]?.path) {
    ...
    // Minimal legacy tool aliases retained for historical message rendering.
    'search': {
    ...
    'edit': {
        title: (opts: { metadata: Metadata | null, tool: ToolCall }) => {
            // Historical payloads may nest data; try the known read-only locations.
    ...
    'execute': {
        title: (opts: { metadata: Metadata | null, tool: ToolCall }) => {
            // Historical payloads may provide a display title in toolCall.title.
