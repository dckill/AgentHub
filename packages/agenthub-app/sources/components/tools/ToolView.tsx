...
import { Text, View, TouchableOpacity, ActivityIndicator, Platform, StyleSheet as RNStyleSheet } from 'react-native';
...
import { StatusChip } from '@/components/glass';
import { getToolStateLabel, getToolStateTone, getToolSurfaceVisuals } from '@/components/messageSurfaceVisuals';
...
export const ToolView = React.memo<ToolViewProps>((props) => {
    ...
    const toolVisuals = getToolSurfaceVisuals(theme);
    ...
    const handlePress = React.useCallback(() => {
        if (onPress) {
            ...
            router.push(`/session/${sessionId}/file?path=${encodeSessionFileRoutePath(filePath)}`);
            ...
            {isPressable ? (
                <TouchableOpacity
                    style={[
                        ...
                        {
                            padding: s(12),
                            backgroundColor: toolVisuals.headerBackgroundColor,
                            borderColor: toolVisuals.innerBorderColor,
                            borderBottomColor: toolVisuals.dividerColor,
                        },
                    ]}
                    onPress={handlePress}
                    activeOpacity={0.8}
                >
                    <View style={[styles.headerLeft, { gap: s(8) }]}>
                        ...
                        <View style={styles.titleContainer}>
                            <Text style={[styles.toolName, { fontSize: s(14) }]} numberOfLines={1}>{toolTitle}{status ? <Text style={[styles.status, { fontSize: s(15) }]}>{` ${status}`}</Text> : null}</Text>
                            {description && (
                                <Text style={[styles.toolDescription, { fontSize: s(13), marginTop: s(2) }]} numberOfLines={1}>
                                    {description}
                                </Text>
                            )}
                        </View>
                        {tool.state === 'running' && (
                            <View style={[styles.elapsedContainer, { marginLeft: s(8) }]}>
                                <ElapsedView from={tool.createdAt} s={s} />
                            </View>
                        )}
                        <StatusChip label={getToolStateLabel(tool.state)} tone={getToolStateTone(tool.state)} style={{ height: s(23), paddingHorizontal: s(7) }} />
                        {statusIcon}
                    </View>
                </TouchableOpacity>
            ) : (
                        ...
                        <View
                            style={[
                                styles.content,
                                {
                                    ...
                                    paddingTop: s(8),
                                    backgroundColor: toolVisuals.contentBackgroundColor,
                                    borderColor: toolVisuals.innerBorderColor,
                                },
                            ]}
                        >
                    ...
                    >
                        ...
                        {tool.input && (
                            <ToolSectionView title={t('toolView.input')}>
                                <CodeView code={JSON.stringify(tool.input, null, 2)} language="json" />
                            </ToolSectionView>
                        )}
                        ...
                        {tool.state === 'completed' && tool.result && (
                            <ToolSectionView title={t('toolView.output')}>
                                <CodeView
                                    ...
                                    language={typeof tool.result === 'string' ? 'text' : 'json'}
                                />
                            </ToolSectionView>
                        )}
                    </View>
...
function ElapsedView(props: { from: number, s: (base: number) => number }) {
    const { from, s } = props;
    const elapsed = useElapsedTime(from);
    return <Text style={[styles.elapsedText, { fontSize: s(13) }]}>{elapsed.toFixed(1)}s</Text>;
}
