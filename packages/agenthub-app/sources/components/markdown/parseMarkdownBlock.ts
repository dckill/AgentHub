...
export function parseMarkdownBlock(markdown: string) {
    ...
    outer: while (index < lines.length) {
        ...
        if (blockquoteMatch) {
            const allLines = [blockquoteMatch[1]];
            while (index < lines.length) {
                const nextMatch = lines[index].trim().match(/^>\s?(.*)$/);
                if (!nextMatch) break;
                allLines.push(nextMatch[1]);
                index++;
            }
            blocks.push({ type: 'blockquote', content: parseMarkdownSpans(allLines.join('\n').trim(), false) });
            continue;
        }
        ...
        if (taskListMatch) {
            let allLines = [{
                ...
                content: trimmed.slice(taskListMatch[0].length),
            }];
            while (index < lines.length) {
            ...
            blocks.push({ type: 'task-list', items: allLines.map((l) => ({ checked: l.checked, spans: parseMarkdownSpans(l.content, false) })) });
            continue;
        }
