...
import { buildGitFileDiffExec, buildGitFileReadExec } from '@/utils/gitDiffCommand';
...
export const InlineFileDiff = React.memo(function InlineFileDiff({ sessionId, fullPath, status, onClose }: InlineFileDiffProps) {
    ...
    React.useEffect(() => {
        ...
        (async () => {
            ...
            try {
                if (status === 'untracked') {
                    const res = await sessionExec(sessionId, {
                        ...buildGitFileReadExec(gitDiffPath),
                        cwd: sessionPath,
                        timeout: 5000,
                    });
                    if (cancelled) return;
                ...
                const res = await sessionExec(sessionId, {
                    ...buildGitFileDiffExec(gitDiffPath, null),
                    cwd: sessionPath,
                    timeout: 5000,
                });
                if (cancelled) return;
...
const DiffPaneHeader = React.memo(function DiffPaneHeader({
    ...
    return (
        <View style={[styles.paneHeader, { backgroundColor: theme.colors.surfaceHigh, borderBottomColor: theme.colors.divider }]}>
            <FileIcon fileName={fileName} size={27} />
            <Text
                numberOfLines={1}
