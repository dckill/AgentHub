...
import { createQRMatrix, type QRMatrix } from './qrMatrix';
import { t } from '@/text';
...
export const QRCode = React.memo((props: QRCodeProps) => {
    ...
    const [qrMatrix, setQrMatrix] = React.useState<QRMatrix | null>(null);
    ...
    React.useEffect(() => {
        ...
        void Promise.resolve(createQRMatrix(data, errorCorrectionLevel)).then((matrix) => {
            if (active) setQrMatrix(matrix);
        }).catch(() => {
    ...
    const moduleSize = qrMatrix ? size / (qrMatrix.size + 4/* space around */) : 0;

    // Generate modules with rounded corners
    const modules = React.useMemo(() => {
        const elements: React.ReactElement[] = [];
        if (!qrMatrix) return elements;
    ...
    if (loadError) {
        return (
            ...
            >
                <Text style={{ color: foregroundColor }}>{t('common.error')}</Text>
            </View>
        );
    }
