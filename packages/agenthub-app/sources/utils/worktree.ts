...
import { buildGitWorktreeRemoveExec } from './gitDiffCommand';
...
const MANAGED_WORKTREE_NAME = /^[A-Za-z0-9][A-Za-z0-9._-]*$/;

export function parseManagedWorktreePath(
    worktreePath: string,
): { basePath: string; worktreeName: string } | null {
    ...
    const worktreeName = worktreePath.slice(idx + WORKTREE_PATH_MARKER.length);
    if (!MANAGED_WORKTREE_NAME.test(worktreeName) || worktreeName === '.' || worktreeName === '..') {
        return null;
    }
...
export async function createWorktree(
    ...
    const gitCheck = await machineExec(
        machineId,
        { executable: 'git', args: ['rev-parse', '--git-dir'], cwd: basePath },
    );
    ...
    let result = await machineExec(
        machineId,
        { executable: 'git', args: ['worktree', 'add', '-b', name, worktreePath], cwd: basePath },
    );
    ...
    if (!result.success && result.stderr?.includes('already exists')) {
        ...
        for (let i = 2; i <= 4; i++) {
            ...
            result = await machineExec(
                machineId,
                { executable: 'git', args: ['worktree', 'add', '-b', newName, newWorktreePath], cwd: basePath },
            );
...
export async function listWorktrees(
    ...
    const result = await machineExec(
        machineId,
...
export async function removeWorktree(
    ...
    const managedPath = parseManagedWorktreePath(worktreePath);
    if (!managedPath) {
    ...
    const result = await machineExec(
        machineId,
        { ...buildGitWorktreeRemoveExec(worktreePath), cwd: managedPath.basePath },
    );
    return {
