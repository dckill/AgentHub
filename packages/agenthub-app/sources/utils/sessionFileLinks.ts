...
export function encodeSessionFileRoutePath(path: string): string {
    return encodeURIComponent(base64EncodeUtf8(path));
}

export function decodeSessionFileRoutePath(encodedPath: string): string {
    try {
        return base64DecodeUtf8(decodeURIComponent(encodedPath));
    } catch {
        try {
