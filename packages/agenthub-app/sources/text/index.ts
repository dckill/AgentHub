 ...
 * English remains resident as the synchronous fallback. Platform loaders own
 * the complete typed mapping for the other supported languages.
 */
const translations: Partial<Record<SupportedLanguage, TranslationStructure>> = {
    en,
};

const logI18n = (...args: unknown[]) => {
    if (shouldLogI18n()) {
        console.log(...args);
    }
};
...
let found = false;
if (settings.settings.preferredLanguage && SUPPORTED_LANGUAGE_CODES.includes(settings.settings.preferredLanguage as SupportedLanguage)) {
    currentLanguage = settings.settings.preferredLanguage as SupportedLanguage;
    found = true;
    logI18n(`[i18n] Using preferred language: ${currentLanguage}`);
}

// Read from device
if (!found) {
    let locales = Localization.getLocales();
    logI18n(`[i18n] Device locales:`, locales.map(l => l.languageCode));
    for (let l of locales) {
        if (l.languageCode) {
            ...
            if (l.languageCode === 'zh') {
                ...
                logI18n(`[i18n] Chinese script code: ${l.languageScriptCode} -> ${chineseVariant}`);

                if (chineseVariant && SUPPORTED_LANGUAGE_CODES.includes(chineseVariant as SupportedLanguage)) {
                    currentLanguage = chineseVariant as SupportedLanguage;
                    logI18n(`[i18n] Using Chinese variant: ${currentLanguage}`);
                    break;
                }

                currentLanguage = 'zh-Hans';
                logI18n(`[i18n] Falling back to simplified Chinese: zh-Hans`);
                break;
            }

            // Direct match for non-Chinese languages
            if (SUPPORTED_LANGUAGE_CODES.includes(l.languageCode as SupportedLanguage)) {
                currentLanguage = l.languageCode as SupportedLanguage;
                logI18n(`[i18n] Using device locale: ${currentLanguage}`);
                break;
            }
        }
    }
...
logI18n(`[i18n] Final language: ${currentLanguage}`);
applyWebDocumentLanguage(currentLanguage);

export async function loadCurrentTranslations(): Promise<void> {
    ...
    try {
        ...
        console.warn(`[i18n] Failed to load ${currentLanguage}; falling back to English`, error);
        currentLanguage = 'en';
        applyWebDocumentLanguage(currentLanguage);
    }
}
