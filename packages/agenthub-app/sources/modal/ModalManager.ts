...
class ModalManagerClass implements IModal {
    ...
    alert(title: string, message?: string, buttons?: AlertButton[]): void {
        if (shouldUseCustomModal(Platform.OS, 'alert')) {
            // Show custom modal on web and Android to keep interaction style consistent.
            if (!this.showModalFn) {
    ...
    async confirm(
        ...
        if (shouldUseCustomModal(Platform.OS, 'confirm')) {
            // Show custom modal on web and Android to keep interaction style consistent.
            if (!this.showModalFn) {
                console.error('ModalManager not initialized. Make sure ModalProvider is mounted.');
                return false;
    ...
    async prompt(
        ...
        if (!shouldUseCustomModal(Platform.OS, 'prompt') && !options?.inputType) {
