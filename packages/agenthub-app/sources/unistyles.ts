import './polyfills/screenOrientation';
import { StyleSheet, UnistylesRuntime } from 'react-native-unistyles';
import { darkTheme, lightTheme } from './theme';
import { getAgentHubRootBackground } from './theme/agenthubTheme';
...
const setRootBackgroundColor = () => {
    if (themePreference === 'adaptive') {
        const systemTheme = Appearance.getColorScheme();
        const color = getAgentHubRootBackground(systemTheme === 'dark' ? 'dark' : 'light');
        UnistylesRuntime.setRootViewBackgroundColor(color);
        SystemUI.setBackgroundColorAsync(color);
    } else {
        const color = getAgentHubRootBackground(themePreference === 'dark' ? 'dark' : 'light');
        UnistylesRuntime.setRootViewBackgroundColor(color);
        SystemUI.setBackgroundColorAsync(color);
    }
};
...
if (Platform.OS === 'web' && themePreference === 'adaptive') {
    document.addEventListener('visibilitychange', () => {
        if (document.visibilityState === 'visible') {
            ...
            const color = getAgentHubRootBackground(themeName);
            UnistylesRuntime.setRootViewBackgroundColor(color);
        }
    });
}
