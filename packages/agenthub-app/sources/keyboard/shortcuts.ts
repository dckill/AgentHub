...
export const SESSION_ACTION_SHORTCUTS: Readonly<Record<SessionActionShortcutId, ShortcutChord>> = {
    ...
    'fork-session': { key: 'f', code: 'KeyF', keyLabel: 'F', altKey: true },
    'duplicate-session': { key: 'd', code: 'KeyD', keyLabel: 'D', altKey: true, shiftKey: true },
...
export const GLOBAL_SHORTCUTS: ReadonlyArray<{ id: GlobalShortcutId; key: string; code: string; keyLabel: string }> = [
    { id: 'commandPalette', key: 'k', code: 'KeyK', keyLabel: 'K' },
    { id: 'newSession', key: 'n', code: 'KeyN', keyLabel: 'N' },
    { id: 'settings', key: ',', code: 'Comma', keyLabel: ',' },
];

function hasValidPrimaryModifier(event: KeyboardShortcutEvent, browserSafe: boolean): boolean {
...
export function getGlobalShortcutId(event: KeyboardShortcutEvent, browserSafe = false): GlobalShortcutId | null {
    if (!hasValidPrimaryModifier(event, browserSafe)) return null;
    return GLOBAL_SHORTCUTS.find((shortcut) => event.code
        ? shortcut.code === event.code
        : shortcut.key === event.key.toLowerCase())?.id ?? null;
}

export function getRecentSessionShortcutIndex(event: KeyboardShortcutEvent, browserSafe = false): number | null {
    if (!hasValidPrimaryModifier(event, browserSafe)) return null;
...
export function formatShortcut(modifier: ShortcutModifier, keyLabel: string, browserSafe = false): string {
    if (modifier === 'meta') return `${browserSafe ? '⌥' : ''}⌘${keyLabel}`;
    return `Ctrl${browserSafe ? '+Alt' : ''}+${keyLabel}`;
}
...
export function formatShortcutChord(modifier: ShortcutModifier, chord: ShortcutChord): string {
    ...
    return ['Ctrl', chord.altKey ? 'Alt' : null, chord.shiftKey ? 'Shift' : null, chord.keyLabel]
        .filter(Boolean)
        .join('+');
}

export function matchesShortcutChord(event: KeyboardShortcutEvent, modifier: ShortcutModifier, chord: ShortcutChord): boolean {
    if (event.isComposing || event.repeat || event.getModifierState?.('AltGraph')) return false;
    const hasPrimary = modifier === 'meta'
        ? event.metaKey && !event.ctrlKey
