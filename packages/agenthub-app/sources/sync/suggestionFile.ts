...
class FileSearchCache {
    ...
    private async initializeFuse(cache: SessionCache): Promise<void> {
    ...
    private async ensureCacheValid(sessionId: string): Promise<void> {
        ...
        await cache.refreshLock.inLock(async () => {
            ...
            await this.initializeFuse(cache);
    ...
    async loadAllFiles(sessionId: string): Promise<FileItem[]> {
        await this.ensureCacheValid(sessionId);
        return this.getAllFiles(sessionId);
    }
...
export async function loadAllFiles(sessionId: string): Promise<FileItem[]> {
    return fileSearchCache.loadAllFiles(sessionId);
}
