...
import { machineExec, sessionExec } from './ops';
...
export async function getGitStatusFiles(sessionId: string): Promise<GitStatusFiles | null> {
    try {
        ...
        const runSessionExec = async (args: string[]) => sessionExec(sessionId, {
            executable: 'git',
            args,
            cwd: metadata.path,
            timeout: 10000,
        ...
        const runGit = async (args: string[]) => {
            const sessionResult = await runSessionExec(args);
            if (sessionResult.success || sessionResult.exitCode === 0) {
                return sessionResult;
            }

            if (metadata.machineId) {
                const machineResult = await machineExec(metadata.machineId, {
                    executable: 'git', args, cwd: metadata.path, timeout: 10000,
                });
                if (machineResult.success || machineResult.exitCode === 0) {
                    return machineResult;
                }
                return machineResult;
            }

            return runSessionExec(args);
        };
        ...
        if (!statusResult.success || statusResult.exitCode !== 0) {
            ...
            return {
                ...
                totalStaged: 0,
                totalUnstaged: 0,
                debugError: statusResult.stderr || `exitCode=${statusResult.exitCode}`,
            };
        }
        ...
        const diffOutput = `${unstagedDiff.success ? (unstagedDiff.stdout ?? '') : ''}\n---STAGED---\n${stagedDiff.success ? (stagedDiff.stdout ?? '') : ''}`;
    ...
    for (const file of statusSummary.files) {
        ...
        if (file.index !== ' ' && file.index !== '.' && file.index !== '?') {
            ...
            stagedFiles.push({
                ...
                oldPath: file.from,
                headOid: file.hashHead,
                indexOid: file.hashIndex,
            });
        }
