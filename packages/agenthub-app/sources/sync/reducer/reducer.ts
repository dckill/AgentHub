...
type PendingToolResult = {
    ...
    isError: boolean;
    createdAt: number;
    permissions?: ToolResultPermissions;
};
...
export type ReducerState = {
    ...
    sourceTextIdToInternalId: Map<string, string>;
    pendingReadyIdsByFinalTextId: Map<string, Set<string>>;
...
export function createReducer(): ReducerState {
    return {
        ...
        sourceTextIdToInternalId: new Map(),
        pendingReadyIdsByFinalTextId: new Map(),
...
export function reducer(state: ReducerState, messages: NormalizedMessage[], agentState?: AgentState | null): ReducerResult {
    ...
    for (const msg of nonSidechainMessages) {
        ...
        // Ready events close thinking and remain in the message projection as
        // invisible completion provenance for grouping completed turns.
        if (msg.role === 'event' && msg.content.type === 'ready') {
            // Reserve the source id immediately so repeated ready envelopes in
            // this same batch are rejected by the standard deduplication check.
        ...
        if (msg.role === 'user') {
            ...
            for (let c of msg.content) {
                if (c.type === 'text' || c.type === 'thinking') {
                    ...
                    if (!isThinking) {
                        state.sourceTextIdToInternalId.set(msg.id, mid);
                        const pendingReadyIds = state.pendingReadyIdsByFinalTextId.get(msg.id);
                        if (pendingReadyIds) {
                            for (const readyId of pendingReadyIds) {
                                const readyMessage = state.messages.get(readyId);
                                if (!readyMessage) continue;
                                readyMessage.meta = { ...readyMessage.meta, finalTextId: mid };
                                changed.add(readyMessage.id);
                            }
                            state.pendingReadyIdsByFinalTextId.delete(msg.id);
                        }
                    }
                }
            }
                ...
                if (c.type === 'tool-call') {
                    ...
                    if (existingMessageId) {
                        ...
                        if (message?.tool) {
                            ...
                            if (pendingResult && applyToolResultToMessage(message, c.id, pendingResult)) {
                                state.pendingToolResults.delete(c.id);
                                if (message.tool.name === 'TodoWrite' && !pendingResult.isError) {
                                    updateLatestTodos(state, message.tool.result?.newTodos, pendingResult.createdAt);
                                }
                            }
                            changed.add(existingMessageId);
                        ...
                        let mid = allocateId();
                        const reducerMessage: ReducerMessage = {
                            id: mid,
                            realID: msg.id,
                            role: 'agent',
                            createdAt: msg.createdAt,
                        ...
                        state.messages.set(mid, reducerMessage);

                        state.toolIdToMessageId.set(c.id, mid);
                        const pendingResult = state.pendingToolResults.get(c.id);
                        if (pendingResult && applyToolResultToMessage(reducerMessage, c.id, pendingResult)) {
                            state.pendingToolResults.delete(c.id);
                            if (reducerMessage.tool?.name === 'TodoWrite' && !pendingResult.isError) {
                                updateLatestTodos(state, reducerMessage.tool.result?.newTodos, pendingResult.createdAt);
                            }
                        }
                        changed.add(mid);
        ...
        if (msg.role === 'agent') {
            for (let c of msg.content) {
                if (c.type === 'tool-result') {
                    const pendingResult = toPendingToolResult(c.content, c.is_error, msg.createdAt, c.permissions);
                    ...
                    if (!applyToolResultToMessage(message, c.tool_use_id, pendingResult)) {
                        continue;
                    }
    ...
    for (let msg of nonSidechainMessages) {
        if (msg.role === 'event') {
            let mid = allocateId();
            const sourceFinalTextId = msg.content.type === 'ready' ? msg.meta?.finalTextId : undefined;
            const internalFinalTextId = sourceFinalTextId
                ? state.sourceTextIdToInternalId.get(sourceFinalTextId)
                : undefined;
            state.messages.set(mid, {
                id: mid,
                realID: msg.id,
                ...
                tool: null,
                text: null,
                meta: internalFinalTextId
                    ? { ...msg.meta, finalTextId: internalFinalTextId }
                    : msg.meta,
            });
            if (sourceFinalTextId && !internalFinalTextId && msg.meta?.turnStatus === 'completed') {
                const pendingReadyIds = state.pendingReadyIdsByFinalTextId.get(sourceFinalTextId) ?? new Set<string>();
                pendingReadyIds.add(mid);
                state.pendingReadyIdsByFinalTextId.set(sourceFinalTextId, pendingReadyIds);
            }
            state.messageIds.set(msg.id, mid);
            changed.add(mid);
        }
    ...
    return {
        ...
        usage: state.latestUsage ? {
            ...
            ...(state.latestUsage.contextWindow !== undefined ? { contextWindow: state.latestUsage.contextWindow } : {})
        } : undefined,
...
function processUsageData(state: ReducerState, usage: UsageData, timestamp: number) {
    ...
    if (!state.latestUsage || timestamp > state.latestUsage.timestamp) {
        state.latestUsage = {
            ...
            ...('context_window' in usage && typeof usage.context_window === 'number' ? { contextWindow: usage.context_window } : {}),
            timestamp: timestamp
        };
    }
}
