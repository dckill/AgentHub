...
describe('reducer', () => {
    ...
    describe('user message handling', () => {
        ...
        it('should preserve user image metadata for rendered message bubbles', () => {
            const state = createReducer();
            const messages: NormalizedMessage[] = [
                {
                    id: 'msg-images',
    ...
    describe('session protocol lifecycle and subagent sidechains', () => {
        it('persists ready events as invisible messages while setting hasReadyEvent', () => {
            const state = createReducer();
            const result = reducer(state, [{
                id: 'ready-1',
                localId: null,
        ...
        it('deduplicates repeated ready envelopes within the same reducer batch', () => {
            const state = createReducer();
            const ready: NormalizedMessage = {
                id: 'ready-duplicate',
                localId: null,
                createdAt: 1000,
                role: 'event',
                content: { type: 'ready' },
                isSidechain: false,
                meta: { turnStatus: 'completed', finalTextId: 'source-final' },
            };

            const result = reducer(state, [ready, { ...ready }]);

            expect(result.messages.filter((message) => message.kind === 'agent-event')).toHaveLength(1);
            expect(state.messageIds.get(ready.id)).toBe(result.messages[0]?.id);
        });

        it('remaps persisted ready provenance when an older page loads the final text later', () => {
            const state = createReducer();
            const ready = reducer(state, [{
                id: 'ready-page',
                localId: null,
                createdAt: 2000,
                role: 'event',
            ...
            expect(ready.messages[0]).toMatchObject({
                kind: 'agent-event',
                meta: { finalTextId: 'source-final' },
            });
            expect(state.pendingReadyIdsByFinalTextId.get('source-final')).toEqual(new Set([ready.messages[0]?.id]));

            const olderPage = reducer(state, [{
                id: 'source-final',
                localId: null,
                createdAt: 1000,
                role: 'agent',
                content: [{ type: 'text', text: 'Final answer', uuid: 'source-final', parentUUID: null }],
                isSidechain: false,
            }]);
            const text = olderPage.messages.find((message) => message.kind === 'agent-text');
            const remappedReady = olderPage.messages.find((message) => message.kind === 'agent-event');
            expect(remappedReady?.meta?.finalTextId).toBe(text?.id);
            expect(state.sourceTextIdToInternalId.get('source-final')).toBe(text?.id);
            expect(state.pendingReadyIdsByFinalTextId.has('source-final')).toBe(false);
        ...
        it('indexes 10k text projections without scanning the message map per text', () => {
            const state = createReducer();
            const valuesSpy = vi.spyOn(state.messages, 'values');
            const messages: NormalizedMessage[] = Array.from({ length: 10_000 }, (_, index) => ({
                id: `source-${index}`,
                localId: null,
                createdAt: index,
                role: 'agent',
            ...
            expect(state.sourceTextIdToInternalId.size).toBe(10_000);
