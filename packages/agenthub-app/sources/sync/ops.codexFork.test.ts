...
vi.mock('./sync', () => ({
    sync: { refreshSessions, getCredentials: vi.fn(() => null) },
}));
...
describe('codex fork ops', () => {
    ...
    it('forks a full Codex thread and spawns a Codex session resumed to the new thread', async () => {
        machineRPC.mockImplementation(async (_machineId: string, method: string) => {
            ...
            if (method === 'spawn-agenthub-session') {
                return { type: 'success', sessionId: 'agenthub-forked' };
            }
        ...
        const result = await forkAndSpawn({
            kind: 'codex',
            sessionId: 'agenthub-source',
            machineId: 'machine-1',
            directory: '/tmp/project',
            codexThreadId: 'thread-source',
        });

        expect(result).toEqual({ type: 'success', sessionId: 'agenthub-forked' });
        ...
        expect(machineRPC).toHaveBeenNthCalledWith(
            ...
            expect.objectContaining({
                ...
                parentSessionId: 'agenthub-source',
            }),
        );
    ...
    it('duplicates a Codex thread from a selected user item before spawning', async () => {
        machineRPC.mockImplementation(async (_machineId: string, method: string) => {
            ...
            if (method === 'spawn-agenthub-session') {
                return { type: 'success', sessionId: 'agenthub-cut' };
            }
            throw new Error(`unexpected method ${method}`);
        });
        ...
        expect(result).toEqual({ type: 'success', sessionId: 'agenthub-cut' });
        expect(machineRPC).toHaveBeenNthCalledWith(
            1,
            'machine-1',
