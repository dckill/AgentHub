...
describe('settings', () => {
    describe('settingsParse', () => {
        ...
        it('enables automatic agent work collapsing by default', () => {
            expect(settingsDefaults.collapseAgentWork).toBe(true);
        ...
        it('keeps the legacy tool grouping preference without using it as the new default', () => {
            const parsed = settingsParse({ groupToolCalls: false });

            expect(parsed.groupToolCalls).toBe(false);
            expect(parsed.collapseAgentWork).toBe(true);
        });
