...
// ============================================================================
// Accepts both hyphenated (Codex) and underscore (Claude Code) formats
...
/**
 * Hyphenated tool-call format from Codex
...
/**
 * Hyphenated tool-call-result format from Codex
...
const rawAgentRecordSchema = z.discriminatedUnion('type', [z.object({
    ...
    // Legacy read-only envelope retained for decrypting historical records.
    type: z.literal('acp'),
    ...
    if (envelope.ev.t === 'turn-end') {
        return {
            ...
            meta: {
                ...
                ...(envelope.ev.finalTextId ? { finalTextId: envelope.ev.finalTextId } : {}),
            },
...
export function normalizeRawMessage(id: string, localId: string | null, createdAt: number, raw: RawRecord): NormalizedMessage | null {
    ...
    if (raw.role === 'agent') {
        ...
        // Normalize legacy read-only records; current sessions use sessionProtocol.
