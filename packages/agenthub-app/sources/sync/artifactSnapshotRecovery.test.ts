...
describe('artifact snapshot key recovery', () => {
    it('refreshes the authoritative artifact snapshot after a key decryption miss', () => {
        ...
        const method = source.match(/public fetchArtifactsList[\s\S]*?public async fetchArtifactWithBody/)?.[0] ?? '';

        expect(method).toMatch(/runArtifactListSync/);
        expect(method).toMatch(/scheduleRetry: \(\) => this\.artifactsSync\.invalidate\(\)/);
