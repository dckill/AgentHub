...
export class GitStatusSync {
    ...
    getSync(sessionId: string): InvalidateSync {
        ...
        if (!sync) {
            ...
            sync = new InvalidateSync(() => this.fetchGitStatusForProject(sessionId, projectKey, generation));
    ...
    /** Stop and forget every account-scoped project watcher. */
    resetAll(): void {
        ...
        for (const sync of this.projectSyncMap.values()) {
            sync.stop();
        }
     ...
     */
    private async fetchGitStatusForProject(sessionId: string, projectKey: string, generation: number): Promise<void> {
        try {
            ...
            const gitCheckResult = await sessionExec(sessionId, {
                executable: 'git',
            ...
            const statusResult = await sessionExec(sessionId, {
                executable: 'git',
                args: ['status', '--porcelain=v2', '--branch', '--show-stash', '--untracked-files=all'],
                cwd: session.metadata.path,
                timeout: 10000
            });
            if (generation !== this.lifecycleGeneration) return;
            ...
            const diffStatResult = await sessionExec(sessionId, {
                executable: 'git',
                args: ['diff', '--numstat'],
                cwd: session.metadata.path,
                timeout: 10000
            ...
            const stagedDiffStatResult = await sessionExec(sessionId, {
                executable: 'git',
