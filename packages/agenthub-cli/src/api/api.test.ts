...
describe('Api server error handling', () => {
    ...
    describe('getOrCreateSession', () => {
        it('should return null when AgentHub server is unreachable (ECONNREFUSED)', async () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith(
                expect.stringContaining('⚠️  AgentHub server unreachable')
            );

            consoleSpy.mockRestore();
        });

        it('should return null when AgentHub server cannot be found (ENOTFOUND)', async () => {
        ...
        it('should return null when AgentHub server times out (ETIMEDOUT)', async () => {
            connectionState.reset();
            const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
