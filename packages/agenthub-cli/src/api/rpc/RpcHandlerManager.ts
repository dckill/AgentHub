...
import { createRpcFailure, parseRpcRequest, parseRpcResponse } from '@artsum/agenthub-wire';
    ...
    async handleRequest(
        ...
        try {
            ...
            if (!handler) {
                ...
                const errorResponse = createRpcFailure('METHOD_NOT_FOUND', 'Method not found');
            ...
            const method = request.method.startsWith(`${this.scopePrefix}:`)
                ? request.method.slice(this.scopePrefix.length + 1)
                : request.method;
            let validatedParams: unknown;
            try {
                validatedParams = parseRpcRequest(method, decryptedParams);
            } catch (error) {
                this.logger('[RPC] [ERROR] Invalid request', { method, error });
                return encodeBase64(encrypt(
                    this.encryptionKey,
                    this.encryptionVariant,
                    createRpcFailure('INVALID_REQUEST', `Invalid RPC request: ${method}`),
                ));
            }
            ...
            try {
                validatedResult = parseRpcResponse(method, normalizedResult);
            } catch (error) {
                this.logger('[RPC] [ERROR] Invalid response', { method, error });
                return encodeBase64(encrypt(
                    this.encryptionKey,
                    this.encryptionVariant,
                    createRpcFailure('INVALID_RESPONSE', `Invalid RPC response: ${method}`),
                ));
            }

            // Encrypt and return the response
            const encryptedResponse = encodeBase64(encrypt(this.encryptionKey, this.encryptionVariant, validatedResult));
            this.logger('[RPC] Sending encrypted response', { method: request.method, responseLength: encryptedResponse.length });
            return encryptedResponse;
        } catch (error) {
            this.logger('[RPC] [ERROR] Error handling request', { error });
            const errorResponse = createRpcFailure(
                'INTERNAL_ERROR',
