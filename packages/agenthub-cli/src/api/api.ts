...
export class ApiClient {
  ...
  async getOrCreateSession(opts: {
    ...
    if (this.credential.encryption.type === 'dataKey') {
      ...
      // const contentDataKey = await deriveKey(this.secret, 'AgentHub EnCoder', ['content']);
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.response?.status) {
        ...
        if (status === 403 || status === 409) {
          ...
          console.log(chalk.yellow(
            `   -> Run 'agenthub doctor clean' to reset local state and generate a new machine ID`
          ));
          console.log(chalk.yellow(
   ...
   */
  async registerVendorToken(vendor: 'openai' | 'anthropic', apiKey: any): Promise<void> {
    try {
      const response = await axios.post(
   ...
   */
  async getVendorToken(vendor: 'openai' | 'anthropic'): Promise<any | null> {
    try {
