...
export function resolveCodexExecutionPolicy(
    ...
    sandboxManagedByAgentHub: boolean,
): { approvalPolicy: ApprovalPolicy; sandbox: SandboxMode } {
    if (sandboxManagedByAgentHub) {
        return {
    ...
    const approvalPolicy: ApprovalPolicy = (() => {
        switch (permissionMode) {
            ...
            case 'safe-yolo': return 'never';                      // Auto-run within workspace sandbox
            case 'yolo': return 'never';                           // Auto-run with full access
