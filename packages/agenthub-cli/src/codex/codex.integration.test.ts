...
const DEFAULT_MODEL = process.env.AGENTHUB_CODEX_INTEGRATION_MODEL?.trim() || undefined;
const TURN_SHOULD_SETTLE_MS = 60_000;
...
class CodexDriver {
    ...
    async continueUntilMessageContains(prompt: string, expectedText: string): Promise<string> {
        let text = "";
        for (let attempt = 0; attempt < 2; attempt++) {
            this.clearEvents();
            await this.continue(prompt);
            text = this.getMessages().join(" ").toLowerCase();
...
describe.skipIf(!(await isCodexAppServerAvailable()))(
    ...
    () => {
        ...
        it("should complete turn gracefully after permission cancel", async () => {
            ...
            expect(result.elapsed_ms).toBeLessThan(TURN_SHOULD_SETTLE_MS);
        ...
        it("should preserve context when continuing after cancel", async () => {
            ...
            driver.permissionPolicy = "approve";
            const text = await driver.continueUntilMessageContains(
        ...
        it("should preserve context when continuing after interruptTurn abort", async () => {
            ...
            expect(r2.elapsed_ms).toBeLessThan(TURN_SHOULD_SETTLE_MS);
            ...
            expect(
                text,
                `Codex continuation omitted the expected codename; events=${JSON.stringify(
                    driver.events.map((event) => ({ type: event.type, data: event.data })),
                )}`,
            ).toContain("golden-phoenix-77");
        });
    }
);
