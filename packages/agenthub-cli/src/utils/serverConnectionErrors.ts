...
export interface OfflineReconnectionConfig<TSession> {
    /** Server URL to health-check against (e.g., 'https://agenthub.yzsd.asia:8443') */
    serverUrl: string;
    ...
    const attemptReconnect = async () => {
        ...
        try {
            ...
            if (axios.isAxiosError(e) && e.response?.status === 401) {
                ...
                config.onNotify('❌ Authentication failed. Please re-authenticate with `agenthub auth`.');
...
export const ERROR_DESCRIPTIONS: Record<string, string> = {
    ...
    // HTTP errors
    '401': 'authentication failed - run `agenthub auth`',
...
class OfflineState {
    ...
    private print(): void {
        const summary = [...this.failures.values()]
            ...
            .join('; ');
        console.log(`⚠️  AgentHub server unreachable, offline mode with auto-reconnect enabled - error details: ${summary}`);
