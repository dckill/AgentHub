/**
 * Minimal persistence functions for agenthub CLI
 * 
 * Handles settings and private key storage in ~/.agenthub/ or local .agenthub/
 */
...
import type { ProcessIdentity } from '@/daemon/processIdentity';
import { isSameProcessIdentity, readProcessIdentity } from '@/daemon/processIdentity';
...
interface Settings {
  ...
  daemonAutoStartWhenRunningAgentHub?: boolean
  chromeMode?: boolean
  ...
  try {
    ...
    if (schemaVersion > SUPPORTED_SCHEMA_VERSION) {
      logger.warn(
        ...
        'Update agenthub-cli for full functionality.'
      );
    }
...
export async function writeSettings(settings: Settings): Promise<void> {
  if (!existsSync(configuration.agentHubHomeDir)) {
    await mkdir(configuration.agentHubHomeDir, { recursive: true, mode: 0o700 })
  }
...
export async function updateSettings(
  ...
  while (attempts < MAX_LOCK_ATTEMPTS) {
    try {
      ...
      fileHandle = await open(lockFile, constants.O_CREAT | constants.O_EXCL | constants.O_WRONLY, 0o600);
      break;
    } catch (err: any) {
      if (err.code === 'EEXIST') {
...
export function writeDaemonState(state: DaemonLocallyPersistedState): void {
  atomicWritePrivateJson(configuration.daemonStateFile, state);
}
...
type DaemonLockOwner = { ownerNonce: string; processIdentity: ProcessIdentity };
const lockOwners = new WeakMap<FileHandle, string>();
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      const lockOwner = owner ?? {
        ownerNonce: 'legacy-local-owner',
        processIdentity: (await readProcessIdentity(process.pid))!,
      };
      await fileHandle.writeFile(JSON.stringify(lockOwner));
      await fileHandle.sync();
      lockOwners.set(fileHandle, lockOwner.ownerNonce);
      return fileHandle;
    } catch (error: any) {
      if (error.code === 'EEXIST') {
        // Lock file exists, check if process is still running
        try {
          const lockContent = readFileSync(configuration.daemonLockFile, 'utf-8').trim();
          let stale = false;
          if (lockContent.startsWith('{')) {
            const existing = JSON.parse(lockContent) as DaemonLockOwner;
            const actual = await readProcessIdentity(existing.processIdentity.pid);
            stale = !isSameProcessIdentity(existing.processIdentity, actual);
          } else if (lockContent && !isNaN(Number(lockContent))) {
            try {
              process.kill(Number(lockContent), 0); // Legacy lock compatibility
            } catch {
              stale = true;
            }
          }
          if (stale) {
            ...
            attempt -= 1; // confirmed stale cleanup must still get a real acquisition attempt
            continue;
          }
        } catch {
          // Corrupt/unknown lock ownership fails closed; doctor can surface it.
        }
      }
...
export async function releaseDaemonLock(lockHandle: FileHandle): Promise<void> {
  ...
  try {
    if (existsSync(configuration.daemonLockFile)) {
      const expectedNonce = lockOwners.get(lockHandle);
      const content = readFileSync(configuration.daemonLockFile, 'utf8');
