...
export async function handleSandboxStatus(): Promise<void> {
    ...
    if (!config) {
        console.log('Sandbox is not configured. Run `agenthub sandbox configure`.');
        return;
    }
...
export function handleSandboxHelp(): void {
    console.log(`
${chalk.bold('agenthub sandbox')} - Sandbox management

${chalk.bold('Usage:')}
  agenthub sandbox configure      Configure sandbox settings interactively
  agenthub sandbox status         Show current sandbox configuration
  agenthub sandbox disable        Disable sandboxing
  agenthub sandbox help           Show this help
`);
}
