...
import {
    getLocalAgentHubAgentCredentialPath,
    readLocalAgentHubAgentCredentials,
    type LocalAgentHubAgentCredentials,
} from './localAgentHubAgentAuth';
...
export type ResumableAgentHubSession = {
    id: string;
    active: boolean;
    metadata: Metadata;
};

export type ReconnectableAgentHubSession = ResumableAgentHubSession & {
    seq: number;
    metadataVersion: number;
    agentStateVersion: number;
...
export function resolveSessionRecordByPrefix<T extends { id: string }>(records: T[], sessionId: string): T {
    ...
    if (matches.length === 0) {
        throw new Error(`No AgentHub session found matching "${trimmed}"`);
    }
    if (matches.length > 1) {
        throw new Error(`Ambiguous AgentHub session "${trimmed}" matches ${matches.length} sessions. Be more specific.`);
    }
    return matches[0];
}
...
function readAgentCredentials() {
    const credentialPath = getLocalAgentHubAgentCredentialPath();
    const credentials = readLocalAgentHubAgentCredentials();
    if (!credentials) {
        throw new Error(
            `Cannot resume historical AgentHub sessions without ${credentialPath}. Run \`agenthub-agent auth login\` in this environment first.`,
        );
    }
    return credentials;
}

function resolveSessionEncryption(session: RawSession, credentials: LocalAgentHubAgentCredentials): RecordEncryption {
    if (session.dataEncryptionKey) {
        ...
        if (!sessionKey) {
            throw new Error(`Failed to decrypt data key for AgentHub session ${session.id}`);
        }
        return {
            key: sessionKey,
            variant: 'dataKey',
...
function decryptSessionMetadata(session: RawSession, credentials: LocalAgentHubAgentCredentials): Metadata {
    ...
    if (!metadata) {
        throw new Error(`Failed to decrypt metadata for AgentHub session ${session.id}`);
    }

    try {
        return ResumableMetadataSchema.parse(metadata) as Metadata;
    } catch {
        throw new Error(`AgentHub session ${session.id} is missing resumable metadata.`);
    }
}

async function fetchSessions(credentials: LocalAgentHubAgentCredentials): Promise<RawSession[]> {
    try {
        ...
        if (error instanceof AxiosError) {
            if (error.response?.status === 401) {
                throw new Error('AgentHub session lookup authentication expired. Run `agenthub-agent auth login` in this environment.');
            }
            throw new Error(`Failed to load AgentHub sessions: ${error.message}`);
        }
        throw error;
    }
}

export async function resolveAgentHubSession(sessionId: string): Promise<ResumableAgentHubSession> {
...
export async function resolveReconnectableSession(sessionId: string): Promise<ReconnectableAgentHubSession> {
    const credentials = readAgentCredentials();
    const sessions = await fetchSessions(credentials);
    const matched = resolveSessionRecordByPrefix(sessions, sessionId);
