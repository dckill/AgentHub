...
export async function claudeRemoteLauncher(
    ...
    // Live permission-mode change: apply a mode toggle (e.g. yolo) to the
    // in-flight turn instead of waiting for the next message. The app fires this
    // from the composer selector while the agent is thinking.
    session.client.rpcHandlerManager.registerHandler<{ mode?: string }, { applied: boolean }>(
        'permission-mode',
        async (message) => {
            const mode = message?.mode;
            if (!isPermissionMode(mode)) {
                logger.debug(`[remote]: permission-mode ignored, invalid mode: ${mode}`);
                return { applied: false };
            }
            logger.debug(`[remote]: applying live permission mode: ${mode}`);
            await permissionHandler.applyLiveModeChange(mode);
            return { applied: true };
        }
    );
    ...
    try {
        ...
        // See: https://github.com/oshadow/agenthub/issues/143
        ...
        while (!exitReason) {
            ...
            try {
                const remoteResult = await claudeRemote({
                    ...
                    onQueryReady: (q) => {
                        permissionHandler.setPermissionModeUpdater(async (mode) => {
                            ...
                            // (e.g. yolo -> bypassPermissions) before applying live.
                            await q.setPermissionMode(mapToClaudeMode(mode));
                        });
                    },
                ...
                const action = await handleClaudeBackendFailure({
                    error: e,
                    exitRequested: Boolean(exitReason),
                    onBackendFatal,
