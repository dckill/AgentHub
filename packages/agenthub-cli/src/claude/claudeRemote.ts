...
export async function claudeRemote(opts: {
    ...
    // Dynamic parameters
    nextMessage: () => Promise<{ message: string, mode: EnhancedMode, images?: Array<{ data: string; mimeType: 'image/jpeg' | 'image/png' | 'image/gif' | 'image/webp'; name?: string; width?: number; height?: number }> } | null>,
    onReady: () => void,
    ...
    messages.push({
        ...
        message: {
            role: 'user',
            content: initial.images && initial.images.length > 0
                ? [
                    { type: 'text' as const, text: initial.message },
            ...
            if (message.type === 'result') {
                ...
                opts.nextMessage().then((next) => {
                    if (!next) {
                        ...
                        mode = next.mode;
                        const content = next.images && next.images.length > 0
                            ? [
                                { type: 'text' as const, text: next.message },
                                ...next.images.map(img => ({
                                    type: 'image' as const,
                                    source: {
                                        type: 'base64' as const,
                            ...
                            : next.message;
                        messages.push({ type: 'user', parent_tool_use_id: null, message: { role: 'user', content } });
                    }
                }).catch(() => {
                    messages.end();
                });
