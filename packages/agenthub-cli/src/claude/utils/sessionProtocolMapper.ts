...
export type ClaudeSessionProtocolState = {
    ...
    lastTopLevelTextId?: string | null;
...
function closeTurn(
    ...
    envelopes.push(createEnvelope('agent', {
        t: 'turn-end',
        status,
        ...(status === 'completed' && state.lastTopLevelTextId
            ? { finalTextId: state.lastTopLevelTextId }
            : {}),
    ...
    if (message.type === 'assistant') {
        ...
        for (const block of blocks) {
            if (block.type === 'text' && typeof block.text === 'string') {
                const textEnvelope = createEnvelope('agent', { t: 'text', text: block.text }, { turn: turnId, subagent });
                envelopes.push(textEnvelope);
                if (!subagent) {
                    state.lastTopLevelTextId = textEnvelope.id;
                }
                continue;
            }
    ...
    if (message.type === 'user') {
        ...
        if (!message.isSidechain) {
            const textBlocks = blocks
                .map((block) => block.type === 'text' && typeof block.text === 'string' && block.text.trim().length > 0
                    ? block.text
                    : null)
            ...
            if (block.type === 'tool_result' && typeof block.tool_use_id === 'string' && block.tool_use_id.length > 0) {
                ...
                if (!message.isSidechain) {
                    if (getHiddenParentToolCalls(state).has(block.tool_use_id)) {
                        ...
                        // Emit tool-call-end for the hidden parent tool call so the client
                        // can transition the tool state from 'running' to 'completed'
                        envelopes.push(createEnvelope('agent', {
                            t: 'tool-call-end',
                            call: block.tool_use_id,
                            ...(block.content != null ? { output: block.content } : {}),
