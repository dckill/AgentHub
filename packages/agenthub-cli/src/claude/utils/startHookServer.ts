 ...
 *     │                                         
 *     ├─► generateHookSettingsFile(port) ──► ~/.agenthub/tmp/hooks/session-hook-<pid>.json
 ...
 * 
 * ### Triggered By
 * - `agenthub` (fresh start) - new session created
 * - `agenthub --continue` - continues last session (may fork)
 * - `agenthub --resume` - interactive picker, then resume
 * - `agenthub --resume <id>` - resume specific session
 * - `/compact` command - compacts and forks session
 * - Double-escape fork - user forks conversation in CLI
 * 
 * ### Why Not Use File Watching?
 * File watching has race conditions when multiple AgentHub processes run.
 * With hooks, Claude directly tells THIS specific process about its session,
 * ensuring 1:1 mapping between AgentHub process and Claude session.
 */
