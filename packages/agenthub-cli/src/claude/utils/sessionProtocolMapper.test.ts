...
describe('mapClaudeLogMessageToSessionEnvelopes', () => {
    ...
    it('maps top-level user text blocks to user envelopes instead of assistant output', () => {
        const result = mapClaudeLogMessageToSessionEnvelopes({
            type: 'user',
            uuid: 'u-blocks-1',
            isSidechain: false,
    ...
    it('carries the last top-level Claude text only on successful turn completion', () => {
        const state = { currentTurnId: null };
        const answer = mapClaudeLogMessageToSessionEnvelopes({
            type: 'assistant',
            uuid: 'answer-1',
            message: {
                role: 'assistant',
        ...
        const text = answer.envelopes.find((envelope) => envelope.ev.t === 'text');
        expect(text).toBeTruthy();
        const completed = closeClaudeTurnWithStatus(state, 'completed');
    ...
    it.each(['cancelled', 'failed'] as const)('does not mark %s Claude text as a final answer', (status) => {
        const state = { currentTurnId: null };
        mapClaudeLogMessageToSessionEnvelopes({
            type: 'assistant',
            uuid: 'progress-1',
        ...
        expect(closeClaudeTurnWithStatus(state, status).envelopes.at(-1)?.ev).toEqual({
            t: 'turn-end',
            status,
        });
    });
        ...
        expect(ended.envelopes).toEqual(
            expect.arrayContaining([
                expect.objectContaining({
                    ev: expect.objectContaining({ t: 'tool-call-end', call: 'tool-1', output: 'ok' }),
                }),
            ]),
        );
    });
    ...
    it('emits stop and tool-call-end for completed subagent when parent Task tool returns', () => {
        const mappedSubagent = createId();
        const state = {
            currentTurnId: 'turn-1',
