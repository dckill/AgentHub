...
export class PermissionHandler {
     ...
     *
     * Unlike handleModeChange (which only updates the gate for the next dequeued
     * message), this also pushes the mode into the live SDK query via the
     * updater callback wired by claudeRemote's onQueryReady. This is what makes
     * a composer mode toggle (e.g. switching to yolo) take effect mid-turn
     * instead of only on the next message.
     *
     * Stores the raw mode (same convention as handleModeChange); the updater is
     * responsible for mapping it to the SDK's 4-mode set.
     */
    async applyLiveModeChange(mode: PermissionMode): Promise<void> {
        this.permissionMode = mode;
    ...
    handleToolCall = async (toolName: string, input: unknown, mode: EnhancedMode, options: { signal: AbortSignal; toolUseID: string }): Promise<PermissionResult> => {
        ...
        // This mirrors Claude Code's control-protocol interaction check.
