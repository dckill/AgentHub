/**
 * Integration tests for the direct Claude CLI/query transport surface.
 *
 * Covers:
 *   - clarification + multi-turn context via resume
 *   - real model switching across resumed turns
 *   - AgentHub MCP tool usage (`mcp__agenthub__change_title`)
 ...
 *   - It does not exercise AgentHub's separate local sandbox wrapper; instead it
...
import { startAgentHubServer } from './utils/startAgentHubServer';
...
class ClaudeQueryDriver {
    private agenthubServer: Awaited<ReturnType<typeof startAgentHubServer>> | null = null;
    ...
    async start(): Promise<void> {
        ...
        this.agenthubServer = await startAgentHubServer(fakeSessionClient);
    }

    stop(): void {
        this.agenthubServer?.stop();
...
describe.skipIf(!claudeAvailable)('Claude Integration (SDK/query)', { timeout: 180_000 }, () => {
    ...
    it('should clarify natively, resume across a model switch, use TodoWrite, and cross the project boundary with native tools', async () => {
        ...
        const execution = await driver!.runTurn({
            allowedTools: ['mcp__agenthub__change_title', 'TodoWrite', 'TodoRead', 'Write', 'Edit', 'Read', 'Glob', 'LS'],
            ...
            prompt: [
                ...
                'Then update the agenthub title so it mentions OPTION_B and reply with only DONE.',
            ].join('\n'),
        ...
        expect(execution.toolUseNames).toContain('mcp__agenthub__change_title');
    ...
    it('should leave the file untouched and explain the refusal when native write is explicitly disallowed', async () => {
        const denied = await driver!.runTurn({
            allowedTools: ['mcp__agenthub__change_title', 'Write', 'Edit', 'Read', 'LS'],
