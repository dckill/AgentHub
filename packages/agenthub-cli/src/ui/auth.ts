...
export async function doAuth(): Promise<Credentials | null> {
    ...
    const pollingSecret = encodeBase64(new Uint8Array(randomBytes(32)));
    ...
    try {
        ...
        await axios.post(`${configuration.serverUrl}/v1/auth/request`, {
            ...
            headers: {
                'X-AgentHub-Client': `cli/${configuration.currentCliVersion}`
            }
        });
        if (process.env.DEBUG) {
    ...
    return await doMobileAuth(keypair, pollingSecret);
}

/**
 * Handle mobile authentication flow
 */
async function doMobileAuth(keypair: tweetnacl.BoxKeyPair, pollingSecret: string): Promise<Credentials | null> {
    console.clear();
    console.log('\nMobile Authentication\n');
    console.log('Scan this QR code with your AgentHub mobile app:\n');

    const authUrl = 'agenthub://terminal?' + encodeBase64Url(keypair.publicKey);
    ...
    return await waitForAuthentication(keypair, pollingSecret);
}

/**
 * Wait for authentication to complete and return credentials
 */
async function waitForAuthentication(keypair: tweetnacl.BoxKeyPair, pollingSecret: string): Promise<Credentials | null> {
