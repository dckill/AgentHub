 ...
 * for agenthub CLI including configuration, daemon status, logs, and links
 */
...
import { findAllAgentHubProcesses } from '@/daemon/doctor'
...
export function sanitizeDaemonStateForDisplay(state: DaemonLocallyPersistedState) {
    const {
...
export function getEnvironmentInfo(): Record<string, any> {
    return {
        ...
        AGENTHUB_PROJECT_ROOT: process.env.AGENTHUB_PROJECT_ROOT,
        DANGEROUSLY_LOG_TO_SERVER_FOR_AI_AUTO_DEBUGGING: process.env.DANGEROUSLY_LOG_TO_SERVER_FOR_AI_AUTO_DEBUGGING,
        NODE_ENV: process.env.NODE_ENV,
        DEBUG: process.env.DEBUG,
        workingDirectory: process.cwd(),
        processArgv: process.argv,
        agenthubDir: configuration?.agentHubHomeDir,
...
/**
 * Slim daemon status output for `agenthub daemon status`
 */
export async function runDoctorDaemon(): Promise<void> {
    ...
    try {
        ...
        if (state) {
            ...
            console.log(chalk.gray(JSON.stringify(sanitizeDaemonStateForDisplay(state), null, 2)));
        }
    } catch (error) {
        console.log(chalk.red('❌ Error checking daemon status'));
    }

    console.log(chalk.gray('\nRun `agenthub doctor` for full diagnostics.\n'));
}
...
export async function runDoctorCommand(): Promise<void> {
    ...
    // All AgentHub processes
    try {
        const allProcesses = await findAllAgentHubProcesses();
        if (allProcesses.length > 0) {
            console.log(chalk.bold('All AgentHub CLI Processes'));
            ...
            if (allProcesses.length > 1) {
                console.log(chalk.bold('\n💡 Process Management'));
                console.log(chalk.gray('To clean up runaway processes: agenthub doctor clean'));
            }
        } else {
            console.log(chalk.red('No AgentHub processes found'));
        }
    } catch (error) {
    ...
    const wrapperPath = join(projectRoot, 'bin', 'agenthub.mjs');
    ...
    console.log(`AGENTHUB_HOME_DIR: ${env.AGENTHUB_HOME_DIR ? chalk.green(env.AGENTHUB_HOME_DIR) : chalk.gray('not set')}`);
    console.log(`AGENTHUB_SERVER_URL: ${env.AGENTHUB_SERVER_URL ? chalk.green(env.AGENTHUB_SERVER_URL) : chalk.gray('not set')}`);
    ...
    console.log(chalk.bold('\n🐛 Support & Bug Reports'));
    console.log(`Report issues: ${chalk.blue('https://agenthub.yzsd.asia/support')}`);
    console.log(`Documentation: ${chalk.blue('https://agenthub.yzsd.asia/docs')}`);

    // ── Concise useful info last (visible without scrolling) ──

    // Basic info
    console.log(chalk.bold('\n📋 Basic Information'));
    console.log(`AgentHub CLI Version: ${chalk.green(packageJson.version)}`);
    console.log(`Platform: ${chalk.green(process.platform)} ${process.arch}`);
    console.log(`Node.js Version: ${chalk.green(process.version)}`);

    // Configuration
    console.log(chalk.bold('\n⚙️  Configuration'));
    console.log(`AgentHub Home: ${chalk.blue(configuration.agentHubHomeDir)}`);
