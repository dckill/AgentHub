...
function resolveBundledToolsDir() {
    const prepared = process.env.AGENTHUB_INTERNAL_TOOLS_DIR;
    if (prepared && path.isAbsolute(prepared)) return path.resolve(prepared);
...
function loadRipgrepNative() {
    ...
    const toolsDir = resolveBundledToolsDir();
