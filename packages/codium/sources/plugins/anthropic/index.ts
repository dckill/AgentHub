 ...
 * driven by the chat runner against the worker-hosted AgentHub Claude CLI
 * transport (see boot/main/agent-worker). The plugin itself just exposes models
...
class AnthropicPlugin implements Plugin {
    ...
    async connect(credential: string, ctx: PluginContext): Promise<AuthState> {
        ...
        // Light-weight format check — Claude Code will surface real auth
