 ...
 *
 * Hosts AgentHub's external Claude CLI transport and exposes a small session-oriented
...
import {
    ...
    type SDKUserMessage,
} from '@artsum/agenthub/claude-sdk'
...
function mapSdkMessage(msg: SDKMessage, state: SessionState): AgentEvent[] {
    if (msg.type === 'system' && msg.subtype === 'init') {
        const systemMessage = msg as SDKSystemMessage
        return [{ type: 'session_init', sessionId: systemMessage.session_id ?? '', model: systemMessage.model }]
    }
    ...
    if (msg.type === 'assistant') {
        const assistantMessage = msg as SDKAssistantMessage
    ...
    if (msg.type === 'user') {
        const userMessage = msg as SDKUserMessage
    ...
    if (msg.type === 'result') {
        const resultMessage = msg as SDKResultMessage
