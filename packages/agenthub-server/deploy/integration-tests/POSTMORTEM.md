...
## Setup
- 2 agenthub-server replicas behind a `LoadBalancer` service via `minikube tunnel`
...
[+ 2.85s] killing daemon pod agenthub-server-67b86c7b7c-2bc6f
...
**Observation:** `kubectl exec agenthub-redis-0 -- redis-cli XINFO STREAM socket.io`
...
- **Network-blackout effects:** `iptables -I OUTPUT -d <redis>`
  applied via `kubectl debug --profile=netadmin --target=agenthub` did
...
1. **Re-register on every reconnect** (client side, daemon)
    - Already done by agenthub-cli for fresh connects but the
