...
function redisStats() {
    try {
        const streamLen = execSync(`kubectl exec agenthub-redis-0 -c redis -- redis-cli xlen socket.io 2>/dev/null`).toString().trim();
        const info = execSync(`kubectl exec agenthub-redis-0 -c redis -- redis-cli info all 2>/dev/null`).toString();
...
async function main() {
    ...
    const redis = new Redis({ host: "127.0.0.1", port: REDIS_PORT, lazyConnect: true, maxRetriesPerRequest: 3 });
    await redis.connect();
