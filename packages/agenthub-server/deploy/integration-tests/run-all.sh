...
REDIS_PORT="$(node -e 'const net=require("node:net");const s=net.createServer();s.listen(0,"127.0.0.1",()=>{console.log(s.address().port);s.close()})')"
DEPLOY=false
SAFE_ONLY=false
PF_PIDS=()
SERVER_URL=""
...
cleanup() {
    ...
    pkill -f "port-forward svc/agenthub-server" 2>/dev/null || true
    pkill -f "port-forward svc/agenthub-redis" 2>/dev/null || true
}
trap cleanup EXIT
...
kubectl wait --for=condition=ready pod -l app=agenthub-server --timeout=120s
PODS=$(kubectl get pods -l app=agenthub-server --no-headers | wc -l | tr -d ' ')
echo -e "${GREEN}$PODS server pods ready${NC}"

# --- Server URL: use minikube tunnel for NodePort (real LB, survives pod kills) ---
setup_server_url() {
    local svc_type
    svc_type=$(kubectl get svc agenthub-server -o jsonpath='{.spec.type}' 2>/dev/null)
    ...
    if [ "$svc_type" = "NodePort" ]; then
        ...
        local tmpfile
        tmpfile=$(mktemp)
        minikube service agenthub-server --url > "$tmpfile" 2>/dev/null &
        PF_PIDS+=($!)
        sleep 2
        ...
        kubectl port-forward svc/agenthub-server 13017:3000 &>/dev/null &
        PF_PIDS+=($!)
        SERVER_URL="http://127.0.0.1:13017"
    ...
    kubectl port-forward svc/agenthub-redis "$REDIS_PORT:6379" &>/dev/null &
    PF_PIDS+=($!)
    wait_for_redis_forward
...
wait_for_redis_forward() {
    local forward_pid="${PF_PIDS[${#PF_PIDS[@]}-1]}"
    for _ in {1..40}; do
        if ! kill -0 "$forward_pid" 2>/dev/null; then
            echo -e "${RED}Redis port-forward exited before becoming ready${NC}"
            return 1
        fi
        if node -e 'const net=require("node:net");const s=net.connect(Number(process.argv[1]),"127.0.0.1",()=>{s.end();process.exit(0)});s.setTimeout(250,()=>{s.destroy();process.exit(1)});s.on("error",()=>process.exit(1))' "$REDIS_PORT"; then
            return 0
        fi
        sleep 0.25
    done
    echo -e "${RED}Redis port-forward not reachable on 127.0.0.1:$REDIS_PORT${NC}"
    return 1
}
...
wait_for_pods() {
    ...
    for i in {1..60}; do
        READY=$(kubectl get pods -l app=agenthub-server --no-headers 2>/dev/null | grep -c "Running" || true)
        if [ "$READY" -ge 2 ]; then
            break
        fi
        sleep 1
    done
    kubectl wait --for=condition=ready pod -l app=agenthub-server --timeout=120s 2>/dev/null || true
}
...
run_test "stress-prod-realistic" \
    "SERVER_URL=$SERVER_URL REDIS_PORT=$REDIS_PORT node '$SCRIPT_DIR/stress-prod-realistic.mjs'"
