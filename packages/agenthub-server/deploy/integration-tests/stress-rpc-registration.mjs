...
function getPods() {
    return kc("get pods -l app=agenthub-server -o jsonpath='{.items[*].metadata.name}'")
        .replace(/'/g, "")
        .split(/\s+/)
        .filter(Boolean);
}
...
async function rapidSessions() {
    ...
    // The server intentionally limits each caller socket to 8 in-flight RPCs.
    // Retry only the explicitly backpressured calls so this scenario measures
    // registration loss without treating the safety limit as a routing failure.
    await sleep(2000);
    let retrySuccess = 0, retryFail = 0;
    const retryErrors = new Map();
    const rejectedMethods = results.filter(result => !result.ok).map(result => result.method);
    const retryResults = [];
    for (const [i, method] of rejectedMethods.entries()) {
        const r = await rpcCall(caller, method, `retry-${i}`, 5_000);
        retryResults.push({ method, ...r });
    }
    ...
    log(`After 2s bounded retry: ${retrySuccess}/${rejectedMethods.length} success, ${retryFail}/${rejectedMethods.length} fail`);
    for (const [err, n] of retryErrors) log(`  err: ${err} ×${n}`);

    caller.disconnect();
    daemons.forEach(d => d.daemon.disconnect());

    const onlyBackpressure = [...errors.keys()].every(error => error === "Too many in-flight RPC calls");
    const passed = success + retrySuccess === NUM_DAEMONS && retryFail === 0 && onlyBackpressure;
    log(passed
        ? "✅ PASSED — all daemons reachable; burst overflow was explicitly backpressured"
        : retryFail > 0
            ? "❌ CRITICAL — daemons unreachable even after 2s (permanent registration loss)"
            : "❌ FAILED — burst produced an unexpected non-backpressure error"
    );
    return passed;
}
...
async function rollingDeploy() {
    ...
    const callAllBounded = async (label, timeoutMs) => {
        const groups = callers.map(() => []);
        daemons.forEach((daemon, index) => groups[index % callers.length].push(daemon));
        const results = [];
        await Promise.all(groups.map(async (group, callerIndex) => {
            for (let offset = 0; offset < group.length; offset += 8) {
                const batch = group.slice(offset, offset + 8);
                const batchResults = await Promise.all(
                    batch.map(daemon => rpcCall(callers[callerIndex], daemon.method, label, timeoutMs)),
                );
    ...
    let preOk = 0;
    const preResults = await callAllBounded("pre-kill", 5_000);
    ...
    await sleep(1000);
    let immediateOk = 0;
    const immediateResults = await callAllBounded("immediate", 3_000);
    ...
    if (immediateOk < NUM_DAEMONS * 0.5) {
        ...
        daemons.forEach(d => d.socket.disconnect());
        callers.forEach(c => c.disconnect());
        try { kc("wait --for=condition=ready pod -l app=agenthub-server --timeout=120s"); } catch {}
        return false;
    }
    ...
    for (let sec = 5; sec <= 30; sec += 5) {
        await sleep(5000);
        const results = await callAllBounded(`t+${sec}`, 3_000);
...
// 1. Machine daemon connects + registers spawn-agenthub-session (fire-and-forget)
// 2. "iOS" caller immediately calls spawn-agenthub-session via RPC
...
async function iosSessionFlow() {
    ...
    for (let round = 0; round < ROUNDS; round++) {
        ...
        const MACHINE_METHOD = `${machineId}:spawn-agenthub-session`;
...
async function highConcurrency() {
    ...
    // Keep each caller at the production per-socket budget while all callers
    // run concurrently. This still sustains 40 in-flight calls cluster-wide.
    await Promise.all(callers.map(async caller => {
        for (let offset = 0; offset < daemons.length; offset += 8) {
            const batch = daemons.slice(offset, offset + 8);
            await Promise.all(batch.map(({ method }) =>
...
async function crossReplica3Pod() {
    ...
    log(`Pods: ${pods.join(", ")} (${pods.length} replicas)`);
    if (pods.length < 3) {
        log("⚠️  Need 3+ replicas — skipping (scale up: kubectl scale deployment/agenthub-server --replicas=3)");
        return true;
    }
