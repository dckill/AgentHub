...
Stress tests that run against a real multi-replica AgentHub server on minikube.
...
This builds separate minimal runtime and Prisma migration images, loads both into minikube, applies the kustomize overlay (2-replica server + Postgres + Redis + MinIO), runs migrations only from the migration image, and rolls out the deployment.

After it finishes, open a tunnel to the service:

```bash
minikube service agenthub-server --url
```

Use the printed URL as `SERVER_URL` for the tests below. If port-forwarding instead:

```bash
kubectl port-forward svc/agenthub-server 13017:3000
...
# Tail server logs
kubectl logs -l app=agenthub-server --all-containers -f
...
# Access Redis CLI
kubectl exec -it agenthub-redis-0 -- redis-cli

# Access MinIO console
kubectl port-forward svc/agenthub-minio 9001:9001
