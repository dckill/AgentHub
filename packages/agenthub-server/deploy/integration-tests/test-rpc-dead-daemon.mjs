...
function getPods() {
    return execSync("kubectl get pods -l app=agenthub-server -o jsonpath='{.items[*].metadata.name}'")
...
function portForwardPod(pod, localPort) {
    const child = spawn("kubectl", ["port-forward", pod, `${localPort}:13017`], {
        stdio: "pipe",
        detached: true,
    });
    child.unref();
...
async function testPodKill(token) {
    ...
    try {
        ...
        waitForPodReady("app=agenthub-server", 90);
