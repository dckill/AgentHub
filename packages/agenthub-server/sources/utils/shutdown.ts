...
export async function awaitShutdown() {
    ...
    const executePhase = async (
        phase: string,
        entries: Array<[string, Array<() => Promise<void>>]>,
    ) => {
        const allHandlers: Promise<void>[] = [];
        let totalHandlers = 0;

        for (const [name, handlers] of entries) {
        ...
        log(`Waiting for ${totalHandlers} ${phase} shutdown handlers to complete...`);
        const startTime = Date.now();
        await Promise.all(allHandlers);
        const duration = Date.now() - startTime;
        log(`All ${totalHandlers} ${phase} shutdown handlers completed in ${duration}ms`);
    };

    const entries = [...handlersSnapshot.entries()];
    await executePhase('ingress', entries.filter(([name]) => name === 'api'));
    await executePhase('background', entries.filter(([name]) => name.startsWith('keepAlive:')));
    await executePhase(
        'resource',
        entries.filter(([name]) => name !== 'api' && !name.startsWith('keepAlive:')),
    );
}
