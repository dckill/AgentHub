...
function parseClientLabels(raw: unknown): ClientLabels {
    ...
    const type = raw.trim().split('/')[0].toLowerCase();
    const client = KNOWN_CLIENTS.has(type) ? type : 'unknown';
    return { client, client_type: client };
}

export function normalizeHttpRouteMetricLabel(routeTemplate: string | undefined): string {
    return typeof routeTemplate === 'string' && routeTemplate.length > 0 ? routeTemplate : 'unmatched';
}
...
export function getMetricsLabelsFromSocket(socket: Socket): ClientLabels {
    return parseClientLabels(socket.data.agenthubClient);
}
...
export function getMetricsLabelsFromRequest(request: { headers: Record<string, string | string[] | undefined> }): ClientLabels {
    return parseClientLabels(request.headers['x-agenthub-client']);
}
