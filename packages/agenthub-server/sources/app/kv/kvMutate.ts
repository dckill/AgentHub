...
export async function kvMutate(
    ...
    return await inTx(async (tx) => {
        ...
        const newKeyCount = mutations.filter((mutation) => mutation.version === -1).length;
        if (newKeyCount > 0) {
            const currentKeyCount = await tx.userKVStore.count({ where: { accountId: ctx.uid } });
            if (currentKeyCount + newKeyCount > accountQuotas.kv) {
                throw new AccountQuotaError('kv', accountQuotas.kv);
            }
        }
