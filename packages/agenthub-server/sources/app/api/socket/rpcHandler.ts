 ...
 */
const registeredRpcMetricMethods = new Set<string>(rpcMethodNames);

export function normalizeRpcMetricMethod(prefixedMethod: unknown): string {
    if (typeof prefixedMethod !== 'string' || prefixedMethod.length === 0) return 'unknown';
    const lastColon = prefixedMethod.lastIndexOf(':');
    const baseMethod = lastColon >= 0 ? prefixedMethod.substring(lastColon + 1) : prefixedMethod;
    return registeredRpcMetricMethods.has(baseMethod) ? baseMethod : 'extension';
}
...
export function rpcHandler(userId: string, socket: Socket, io: Server) {
    const inFlightCalls = new ConcurrencyLimiter(8);
    ...
    socket.on('rpc-presence', async (data: any, callback: (response: any) => void) => {
        try {
            const { method } = data ?? {};
            if (!method || typeof method !== 'string') {
            ...
            const targets = await fetchRoomSockets(io, rpcRoom(userId, method), RPC_PRESENCE_FETCH_TIMEOUT_MS, 'presence');
            callback?.({ ok: true, available: targets.some(target => target.id !== socket.id) });
        } catch (error) {
            log({ module: 'websocket', level: 'error' }, `Error in rpc-presence: ${error}`);
    ...
    socket.on('rpc-call', async (data: any, callback: (response: any) => void) => {
        ...
        const finish = (result: string) => {
            ...
            const m = normalizeRpcMetricMethod(method);
        ...
        try {
            ...
            if (!releaseConcurrency) {
                finish('backpressure');
                callback?.({ ok: false, error: 'Too many in-flight RPC calls' });
                return;
            }
            ...
            if (targets.length === 0) {
                targets = await waitForRoomMember(io, room, RPC_RECONNECT_GRACE_MS, normalizeRpcMetricMethod(method));
            }
