    ...
    socket.on('message', async (data: any) => {
        await receiveMessageLock.inLock(async () => {
            try {
                ...
                const result = await appendEncryptedSessionMessages(userId, sid, [{
                    content: message,
                    localId: typeof localId === 'string' ? localId : null
                }]);

                for (const msg of result.createdMessages) {
                    if (!msg.updateSeq) {
                        continue;
                    }
                    const updatePayload = buildNewMessageUpdate(msg, sid, msg.updateSeq, randomKeyNaked(12));
                    eventRouter.emitUpdate({
                        userId,
                        payload: updatePayload,
                        recipientFilter: { type: 'all-interested-in-session', sessionId: sid },
