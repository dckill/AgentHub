...
const kvKeySchema = z.string().min(1).max(MAX_IDENTIFIER_CHARS);
    ...
    app.get('/v1/kv', {
        ...
        schema: {
            querystring: z.object({
                prefix: z.string().max(MAX_IDENTIFIER_CHARS).optional(),
    ...
    app.post('/v1/kv', {
        ...
        schema: {
            body: z.object({
                mutations: z.array(z.object({
                    key: kvKeySchema,
                    value: z.string().max(MAX_KV_VALUE_CHARS).nullable(),
        ...
        try {
            ...
            if (error instanceof AccountQuotaError) {
                return reply.code(429).send({ error: 'quota-exceeded', resource: 'kv', limit: error.limit });
            }
