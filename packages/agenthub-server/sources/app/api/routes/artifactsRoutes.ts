...
import { MAX_ENCRYPTED_BODY_CHARS, MAX_ENCRYPTED_HEADER_CHARS, MAX_ENCRYPTED_KEY_CHARS } from "../utils/validationLimits";
...
export function artifactsRoutes(app: Fastify) {
    ...
    app.post('/v1/artifacts', {
        ...
        schema: {
            body: z.object({
                id: z.string().uuid(),
                header: z.string().max(MAX_ENCRYPTED_HEADER_CHARS),
                body: z.string().max(MAX_ENCRYPTED_BODY_CHARS),
                dataEncryptionKey: z.string().max(MAX_ENCRYPTED_KEY_CHARS)
            }),
            response: {
                200: z.object({
                    id: z.string(),
        ...
        try {
            ...
            let artifact;
            try {
                artifact = await createWithinAccountQuota({
                    resource: 'artifacts',
                    limit: accountQuotas.artifacts,
                    count: (tx) => tx.artifact.count({ where: { accountId: userId } }),
                    create: (tx) => tx.artifact.create({
                        data: {
                            id,
                            accountId: userId,
                ...
                if (error instanceof AccountQuotaError) {
                    return reply.code(429).send({ error: 'quota-exceeded', resource: 'artifacts', limit: error.limit });
                }
                throw error;
            }
    ...
    app.post('/v1/artifacts/:id', {
        ...
        schema: {
            ...
            body: z.object({
                header: z.string().max(MAX_ENCRYPTED_HEADER_CHARS).optional(),
                expectedHeaderVersion: z.number().int().min(0).optional(),
                body: z.string().max(MAX_ENCRYPTED_BODY_CHARS).optional(),
        ...
        try {
            ...
            // Increment seq atomically so independent header/body updates cannot
            // overwrite each other's artifact sequence.
            updateData.seq = { increment: 1 };

            // Compare-and-swap in the database. The earlier read is only useful for
            // returning conflict details; correctness must not depend on it.
            const updateWhere: Prisma.ArtifactWhereInput = { id, accountId: userId };
            if (headerUpdate) updateWhere.headerVersion = expectedHeaderVersion;
            if (bodyUpdate) updateWhere.bodyVersion = expectedBodyVersion;
