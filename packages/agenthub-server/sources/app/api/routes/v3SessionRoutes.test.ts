...
const {
    ...
    const state = {
        ...
        accountSyncEvents: [] as AccountSyncEventRecord[],
    ...
    const accountSyncEventCreate = vi.fn(async (args: any) => {
        const row = args?.data as AccountSyncEventRecord;
        state.accountSyncEvents.push(row);
        return row;
    });
    ...
    const txClient = {
        ...
        sessionMessage: {
            ...
            count: vi.fn(async () => state.messages.length),
...
describe("v3SessionRoutes", () => {
    ...
    it("supports backward pagination from the latest messages", async () => {
        seedSession({ id: "session-1", accountId: "user-1" });
        for (let seq = 1; seq <= 5; seq += 1) {
            seedMessage({ sessionId: "session-1", seq, localId: `l${seq}`, content: { t: "encrypted", c: String(seq) } });
        }
        ...
        app = await createApp();
        const latestPage = await app.inject({
            method: "GET",
            url: "/v3/sessions/session-1/messages?direction=backward&limit=2",
            headers: { "x-user-id": "user-1" }
        });
        const latestBody = latestPage.json();
        expect(latestBody.messages.map((message: any) => message.seq)).toEqual([4, 5]);
        expect(latestBody.hasMore).toBe(true);

        const olderPage = await app.inject({
            method: "GET",
            url: "/v3/sessions/session-1/messages?direction=backward&before_seq=4&limit=2",
            headers: { "x-user-id": "user-1" }
        });
        const olderBody = olderPage.json();
        expect(olderBody.messages.map((message: any) => message.seq)).toEqual([2, 3]);
        expect(olderBody.hasMore).toBe(true);

        const oldestPage = await app.inject({
            method: "GET",
            url: "/v3/sessions/session-1/messages?direction=backward&before_seq=2&limit=2",
            headers: { "x-user-id": "user-1" }
        });
        const oldestBody = oldestPage.json();
        expect(oldestBody.messages.map((message: any) => message.seq)).toEqual([1]);
        expect(oldestBody.hasMore).toBe(false);
    });
    ...
    it("sends a single message and emits a new-message update", async () => {
        ...
        expect(state.accountSyncEvents).toEqual([
            {
                accountId: "user-1",
                seq: 1,
    ...
    it("sends multiple messages with sequential seq numbers", async () => {
        ...
        expect(state.accountSyncEvents.map((event) => event.seq)).toEqual([1, 2, 3]);
    ...
    it("deduplicates by localId and returns mixed existing/new messages sorted by seq", async () => {
        ...
        expect(state.accountSyncEvents.map((event) => event.payload)).toEqual([
            {
                sessionId: "session-1",
                messageId: "msg-2",
                sessionSeq: 2
    ...
    it("enforces send validation limits and auth/session ownership", async () => {
        ...
        const oversizedMessage = await app.inject({
            ...
            url: "/v3/sessions/session-1/messages",
            headers: { "x-user-id": "owner-user" },
            payload: { messages: [{ localId: "large", content: "x".repeat(2 * 1024 * 1024 + 1) }] }
        });
