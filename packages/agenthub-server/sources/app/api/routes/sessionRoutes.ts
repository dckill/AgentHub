...
import { MAX_ENCRYPTED_HEADER_CHARS, MAX_ENCRYPTED_KEY_CHARS, MAX_ENCRYPTED_STATE_CHARS, MAX_IDENTIFIER_CHARS } from "../utils/validationLimits";
...
export function sessionRoutes(app: Fastify) {
        ...
        return reply.send({
            sessions: sessions.map((v) => {
                ...
                return {
                    ...
                    thinking: v.thinking,
                    thinkingAt: v.thinkingAt?.getTime() ?? null,
                    metadata: v.metadata,
    ...
    app.get('/v1/sessions/:sessionId', {
        ...
        const session = await db.session.findFirst({
            where: { accountId: userId, id: sessionId },
            select: {
                id: true,
                seq: true,
                createdAt: true,
    ...
    app.post('/v1/sessions', {
        schema: {
            body: z.object({
                tag: z.string().max(MAX_IDENTIFIER_CHARS),
                metadata: z.string().max(MAX_ENCRYPTED_HEADER_CHARS * 2),
                agentState: z.string().max(MAX_ENCRYPTED_STATE_CHARS).nullish(),
                dataEncryptionKey: z.string().max(MAX_ENCRYPTED_KEY_CHARS).nullish()
            })
        },
        preHandler: app.authenticate
    }, async (request, reply) => {
        ...
        if (session) {
            ...
            let session;
            try {
                session = await createWithinAccountQuota({
                    resource: 'sessions',
                    limit: accountQuotas.sessions,
                    count: (tx) => tx.session.count({ where: { accountId: userId } }),
                    create: async (tx) => {
                        updSeq = await allocateUserSeq(userId, tx);
                ...
                if (error instanceof AccountQuotaError) {
                    return reply.code(429).send({ error: 'quota-exceeded', resource: 'sessions', limit: error.limit });
                }
                throw error;
            }
    ...
    app.post('/v1/sessions/:sessionId/archive', {
        schema: {
            ...
            body: z.union([
                z.object({
                    metadata: z.string().max(MAX_ENCRYPTED_HEADER_CHARS * 2).optional(),
                    expectedMetadataVersion: z.number().int().nonnegative().optional(),
                }).refine(
                    (body) => (body.metadata === undefined) === (body.expectedMetadataVersion === undefined),
                    { message: 'metadata and expectedMetadataVersion must be supplied together' },
                ),
                z.null(),
            ]).optional(),
        },
        ...
        const userId = request.userId;
        const { sessionId } = request.params;
        const archiveMetadata = !request.body || request.body.metadata === undefined
            ? undefined
            : {
        ...
        const result = await db.session.updateMany({
            where: {
                id: sessionId,
                accountId: userId,
                ...(archiveMetadata ? { metadataVersion: archiveMetadata.expectedMetadataVersion } : {}),
            },
            data: {
                active: false,
                ...
                ...(archiveMetadata ? {
                    ...
                    metadataVersion: archiveMetadata.expectedMetadataVersion + 1,
                } : {}),
            }
        });
