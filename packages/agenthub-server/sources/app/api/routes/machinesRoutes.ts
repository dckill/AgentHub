...
import { allocateUserSeq, allocateUserSeqBatch } from "@/storage/seq";
...
export function machinesRoutes(app: Fastify) {
    app.post('/v1/machines', {
        ...
        schema: {
            body: z.object({
                id: z.string().max(MAX_IDENTIFIER_CHARS),
                metadata: z.string().max(MAX_ENCRYPTED_HEADER_CHARS * 2), // Encrypted metadata
                daemonState: z.string().max(MAX_ENCRYPTED_STATE_CHARS).optional(), // Encrypted daemon state
        ...
        if (machine) {
            ...
            let updateSeqs!: number[];
            let newMachine;
            try {
                newMachine = await createWithinAccountQuota({
                    resource: 'machines',
                    limit: accountQuotas.machines,
                    count: (tx) => tx.machine.count({ where: { accountId: userId } }),
                    create: async (tx) => {
                        updateSeqs = await allocateUserSeqBatch(userId, 2, tx);
                ...
                if (error instanceof AccountQuotaError) {
                    return reply.code(429).send({ error: 'quota-exceeded', resource: 'machines', limit: error.limit });
                }
                throw error;
            }

            // Emit both new-machine and update-machine events for backward compatibility
            const [updSeq1, updSeq2] = updateSeqs;
