...
function makeTestConfig(): Config {
    const homeDir = mkdtempSync(join(tmpdir(), 'agenthub-agent-auth-test-'));
    return {
    ...
    describe('authLogin', () => {
        it('completes the auth flow on first poll response', async () => {
            ...
            expect(mockedAxiosPost).toHaveBeenCalledWith(
                ...
                expect.objectContaining({ publicKey: expect.any(String), pollingSecret: expect.any(String) }),
                expectedClientHeaders,
            );
            const pollingSecrets = mockedAxiosPost.mock.calls.map(([, body]) => (body as { pollingSecret: string }).pollingSecret);
            expect(new Set(pollingSecrets).size).toBe(1);
            expect(Buffer.from(pollingSecrets[0], 'base64')).toHaveLength(32);
        });
