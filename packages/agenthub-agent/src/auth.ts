...
export async function authLogin(config: Config): Promise<void> {
    ...
    const pollingSecretBase64 = encodeBase64(getRandomBytes(32));
    ...
    try {
        await axios.post(`${config.serverUrl}/v1/auth/account/request`, {
            ...
            pollingSecret: pollingSecretBase64,
        }, {
            headers: { 'X-AgentHub-Client': 'cli-control-plane/0.1.0' },
        });
    } catch (err) {
    ...
    const qrData = `agenthub:///account?${encodeBase64Url(keypair.publicKey)}`;
    console.log('');
    qrcode.generate(qrData, { small: true }, (code: string) => {
        console.log(code);
    });
    console.log('## Authentication');
    console.log('- Action: Scan this QR code with the AgentHub app');
...
export async function authStatus(config: Config): Promise<void> {
    ...
    if (creds) {
        ...
        console.log('- Action: Run `agenthub-agent auth login` to authenticate this AgentHub device.');
    }
}
