...
const binPath = resolve(packageDir, 'bin', 'agenthub-agent.mjs');
const keepIntegrationEnv = ['1', 'true', 'yes'].includes((process.env.AGENTHUB_AGENT_KEEP_ENV ?? '').toLowerCase());
...
function agentEnvVars(serverPort: number, homeDir: string): NodeJS.ProcessEnv {
    return {
        ...
        AGENTHUB_HOME_DIR: homeDir,
    };
}

function runAgentCli(args: string[], env: NodeJS.ProcessEnv): Promise<string> {
    return new Promise<string>((resolvePromise, rejectPromise) => {
        execFile(process.execPath, [
            ...
            if (error) {
                ...
                rejectPromise(new Error(`${process.execPath} ${[
                    '--no-warnings',
                    '--no-deprecation',
                    binPath,
                    ...args,
...
function createGitProject(envDir: string): { projectDir: string; worktreeDir: string } {
    const projectDir = join(envDir, 'agenthub-agent-test-project');
    const worktreeDir = join(projectDir, '.dev', 'worktree', 'feature-branch');

    mkdirSync(projectDir, { recursive: true });
    runCommand('git', ['init', '--initial-branch=main'], projectDir);
    runCommand('git', ['config', 'user.name', 'AgentHub Agent Test'], projectDir);
    runCommand('git', ['config', 'user.email', 'agenthub-agent-tests@example.com'], projectDir);

    writeFile(join(projectDir, 'README.md'), '# AgentHub Agent Test Project\n');
...
async function waitForSessionInList(sessionId: string, env: NodeJS.ProcessEnv): Promise<void> {
    await waitFor(async () => {
        const sessions = parseJson<Array<{ id: string }>>(await runAgentCli(['list', '--json'], env));
        return sessions.some(session => session.id === sessionId);
    }, 20_000, `session ${sessionId} to appear in agenthub-agent list`);
}

async function waitForHistoryMessage(sessionId: string, expectedText: string, env: NodeJS.ProcessEnv): Promise<void> {
    await waitFor(async () => {
        const history = parseJson<Array<{ content?: unknown }>>(await runAgentCli(['history', sessionId, '--json'], env));
...
async function waitForSessionStatus<T>(
    ...
    await waitFor(async () => {
        lastStatus = parseJson<T>(await runAgentCli(['status', sessionId, '--json'], env));
...
async function approveAgentLogin(
    ...
    const response = await fetch(`${serverUrl}/v1/auth/account/response`, {
        ...
        headers: {
            ...
            'X-AgentHub-Client': 'cli-control-plane/0.1.0',
        },
        body: JSON.stringify({
...
async function runAgentAuthLogin(env: NodeJS.ProcessEnv, approval: { serverUrl: string; token: string; secret: Uint8Array }): Promise<string> {
    return await new Promise<string>((resolvePromise, rejectPromise) => {
        ...
        const timeout = setTimeout(() => {
            ...
            settled = true;
            child.kill('SIGKILL');
            rejectPromise(new Error(`Timed out waiting for agenthub-agent auth login.\n${stdout}\n${stderr}`));
        }, 60_000);
        ...
        child.on('close', code => {
            if (code !== 0) {
                finish(new Error(`agenthub-agent auth login exited with code ${code}\n${stdout}\n${stderr}`));
                return;
            }
...
async function listDaemonSessions(httpPort: number): Promise<Array<{ agentHubSessionId: string; pid: number; startedBy: string }>> {
    ...
    const parsed = await response.json() as { children: Array<{ agentHubSessionId: string; pid: number; startedBy: string }> };
...
describe('agenthub-agent integration', { timeout: 180_000 }, () => {
    beforeAll(async () => {
        ...
        if (keepIntegrationEnv) {
            console.log(`[agenthub-agent integration] keeping environment: ${integrationEnvName}`);
            console.log(`[agenthub-agent integration] environment dir: ${integrationEnvDir}`);
        }
    });
    ...
    it('authenticates, lists machines, and spawns a session through the real daemon RPC path', async () => {
        ...
        const machineOutput = await runAgentCli(['machines'], agentEnv);
        expect(machineOutput).toContain('## Machines');

        const machines = JSON.parse(await runAgentCli(['machines', '--json'], agentEnv)) as Array<{
            id: string;
            active: boolean;
            metadata?: {
                homeDir?: string;
        ...
        await waitFor(async () => {
            const refreshedMachines = JSON.parse(await runAgentCli(['machines', '--json'], agentEnv)) as Array<{
                id: string;
                metadata?: {
                    resumeSupport?: {
                        rpcAvailable?: boolean;
            ...
            return refreshedMachine?.metadata?.resumeSupport?.rpcAvailable === true
                && refreshedMachine.metadata.resumeSupport.agenthubAgentAuthenticated === true;
        ...
        const status = JSON.parse(
            await runAgentCli(['status', sessionId, '--json'], agentEnv),
        ) as {
            id: string;
        ...
        await waitFor(async () => {
            ...
            return sessions.some(session => session.agentHubSessionId === sessionId);
        }, 20_000, 'spawned session to be tracked by daemon');
    });

    it('spawns in the test project root and sends a message through agenthub-agent CLI', async () => {
        if (!activeMachineId || !integrationConfig || !agentHomeDir || !testProjectDir) {
            throw new Error('Integration environment not initialized');
        }

        const agentEnv = agentEnvVars(integrationConfig.serverPort, agentHomeDir);
        const prompt = 'agenthub-agent root message';
        ...
        const sendResult = parseJson<{ sessionId: string; message: string; sent: boolean; permissionMode: string | null }>(
            await runAgentCli(['send', sessionId, prompt, '--json'], agentEnv),
        );
    ...
    it('spawns in a git worktree and sends a message through agenthub-agent CLI', async () => {
        if (!activeMachineId || !integrationConfig || !agentHomeDir || !testWorktreeDir) {
            throw new Error('Integration environment not initialized');
        }

        const agentEnv = agentEnvVars(integrationConfig.serverPort, agentHomeDir);
        const prompt = 'agenthub-agent worktree message';
        const spawnResult = parseJson<{
            type: 'success' | 'requestToApproveDirectoryCreation' | 'error';
            sessionId?: string;
            machineId?: string;
    ...
    it('resumes an existing Codex session through the same daemon RPC used by the app', async () => {
        ...
        const sendResult = parseJson<{ sessionId: string; message: string; sent: boolean; permissionMode: string | null }>(
            await runAgentCli(['send', sourceSessionId, prompt, '--yolo', '--wait', '--json'], agentEnv),
        );
        expect(sendResult).toEqual({
        ...
        const resumeResult = parseJson<{
            ...
            await runAgentCli(['resume', sourceSessionId, '--json'], agentEnv),
        );
        ...
        expect(resumeResult.sessionId).toBe(sourceSessionId);
    ...
    it('spawns Codex, applies yolo permissions via message metadata, and creates a file in the test project', async () => {
        ...
        const sendResult = parseJson<{ sessionId: string; message: string; sent: boolean; permissionMode: string | null }>(
            await runAgentCli(['send', sessionId, prompt, '--yolo', '--wait', '--json'], agentEnv),
        );
        expect(sendResult).toEqual({
            sessionId,
            message: prompt,
