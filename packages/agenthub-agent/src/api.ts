...
import type { SessionMessage as WireSessionMessage } from '@artsum/agenthub-wire';
...
function handleApiError(err: unknown, context: string): never {
    if (err instanceof AxiosError) {
        ...
        if (status === 401) {
            throw new Error('Authentication expired. Run `agenthub-agent auth login` to re-authenticate.');
        }
        if (status === 403) {
...
export async function getSessionMessages(
    ...
    const targetLimit = Math.max(1, Math.floor(options.limit ?? 150));
    const messages: RawMessage[] = [];
    let beforeSeq: number | undefined;

    try {
        while (messages.length < targetLimit) {
            const pageLimit = Math.min(500, targetLimit - messages.length);
            ...
            const data = resp.data as { messages: RawMessage[]; hasMore: boolean };
            messages.unshift(...data.messages);
            if (!data.hasMore || data.messages.length === 0) {
                break;
            }
            beforeSeq = data.messages[0]?.seq;
            if (!beforeSeq) {
                break;
            }
        }
    ...
    return messages.slice(-targetLimit).map(msg => ({
        id: msg.id,
        seq: msg.seq,
