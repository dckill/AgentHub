...
const SUPPORTED_AGENTS: SupportedAgent[] = ['claude', 'codex'];
...
function ensureMachineCanResume(machine: DecryptedMachine): void {
    const metadata = (machine.metadata ?? {}) as {
        resumeSupport?: {
            ...
            agenthubAgentAuthenticated?: unknown;
        };
    };
    ...
    if (metadata.resumeSupport?.agenthubAgentAuthenticated === false) {
        throw new Error('Resume is unavailable on this machine. Run `agenthub-agent auth login` in that machine environment first.');
    }
...
program
    .name('agenthub-agent')
    .description('CLI client for controlling AgentHub agents remotely')
    .version('0.1.0');
...
program
    ...
    .action(async (sessionId: string, opts: { limit?: number; json?: boolean }) => {
        ...
        const creds = requireCredentials(config);
        const session = await resolveSession(config, creds, sessionId);
        const messages = await getSessionMessages(config, creds, session.id, session.encryption, { limit: opts.limit ?? 150 });
