...
function makeConfig(): Config {
    return {
        ...
        homeDir: '/tmp/agenthub-test',
        credentialPath: '/tmp/agenthub-test/agent.key',
    };
}
...
describe('api', () => {
    ...
    describe('listSessions', () => {
        ...
        it('throws on 401 with re-authenticate message', async () => {
            ...
            await expect(listSessions(config, creds)).rejects.toThrow(
                'Authentication expired. Run `agenthub-agent auth login` to re-authenticate.',
            );
        });
    ...
    describe('getSessionMessages', () => {
        it('fetches and decrypts messages with dataKey encryption', async () => {
            ...
            mockedAxios.get.mockResolvedValueOnce({
                data: { messages: rawMessages, hasMore: false },
            });

            const messages = await getSessionMessages(config, creds, 'msg-session', encryption, { limit: 150 });
            ...
            expect(mockedAxios.get).toHaveBeenCalledWith(
                'https://test-server.example.com/v3/sessions/msg-session/messages',
                {
                    headers: expectedAuthHeaders,
                    params: { direction: 'backward', limit: 150 },
                },
            );
        });

        it('paginates backward to satisfy limits above the server page size', async () => {
            const { raw: rawSession, sessionKey } = makeRawSessionWithDataKey(
                creds,
                { path: '/project' },
                null,
            ...
            const makeRawMessage = (seq: number): RawMessage => ({
                id: `msg-${seq}`,
                seq,
                content: { t: 'encrypted', c: encodeBase64(encryptWithDataKey({ seq }, sessionKey)) },
                localId: null,
                createdAt: seq,
                updatedAt: seq,
            });
            ...
            mockedAxios.get
                .mockResolvedValueOnce({
                    data: {
                        messages: Array.from({ length: 500 }, (_, index) => makeRawMessage(index + 501)),
                        hasMore: true,
                    },
                })
                .mockResolvedValueOnce({
                    data: {
                        messages: Array.from({ length: 100 }, (_, index) => makeRawMessage(index + 401)),
                        hasMore: true,
                    },
                });

            const messages = await getSessionMessages(config, creds, 'long-session', encryption, { limit: 600 });

            expect(messages).toHaveLength(600);
            expect(messages.map(message => message.seq)).toEqual(Array.from({ length: 600 }, (_, index) => index + 401));
            expect(mockedAxios.get).toHaveBeenNthCalledWith(
                1,
                'https://test-server.example.com/v3/sessions/long-session/messages',
                {
                    headers: expectedAuthHeaders,
                    params: { direction: 'backward', limit: 500 },
                },
            );
            expect(mockedAxios.get).toHaveBeenNthCalledWith(
                2,
                ...
                {
                    headers: expectedAuthHeaders,
                    params: { direction: 'backward', before_seq: 501, limit: 100 },
                },
            );
        });
        ...
        it('throws on 404 for messages endpoint', async () => {
            ...
            await expect(
                getSessionMessages(config, creds, 'bad-id', encryption, { limit: 150 }),
