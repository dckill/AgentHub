...
function runCli(...args: string[]): { stdout: string; stderr: string; exitCode: number } {
    try {
        const stdout = execFileSync(process.execPath, [
            ...
            binPath,
            ...args,
        ], { encoding: 'utf-8', env: { ...process.env, AGENTHUB_HOME_DIR: '/tmp/nonexistent-agenthub-test' } });
...
describe('agenthub-agent CLI', () => {
    it('should display help output', () => {
        const { stdout } = runCli('--help');
        expect(stdout).toContain('agenthub-agent');
        expect(stdout).toContain('CLI client for controlling AgentHub agents remotely');
        expect(stdout).not.toContain(legacyBrand);
    });
