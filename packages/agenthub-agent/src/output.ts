...
export function formatSessionTable(sessions: DecryptedSession[]): string {
    if (sessions.length === 0) {
        return '## Sessions\n\n- Total: 0\n- Status: No AgentHub sessions yet\n- Next: Start a session from the AgentHub app or run a supported agent through the CLI.';
    }
...
export function formatMachineTable(machines: DecryptedMachine[]): string {
    if (machines.length === 0) {
        return '## Devices\n\n- Total: 0\n- Status: No AgentHub devices yet\n- Next: Authenticate a computer and keep the AgentHub daemon running.';
    }
...
export function formatMessageHistory(messages: DecryptedMessage[]): string {
    if (messages.length === 0) {
        return '## Message History\n\n- Count: 0\n- Status: No AgentHub messages yet';
    }
