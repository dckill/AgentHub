/**
 * CLI smoke tests for agenthub-agent.
 *
...
const binPath = resolve(__dirname, '..', 'bin', 'agenthub-agent.mjs');
...
function runCli(...args: string[]): { stdout: string; stderr: string; exitCode: number } {
    try {
        const stdout = execFileSync(process.execPath, [
            ...
            binPath,
            ...args,
        ], { encoding: 'utf-8', env: { ...process.env, AGENTHUB_HOME_DIR: '/tmp/nonexistent-agenthub-acceptance' } });
...
describe('Smoke: CLI command surface', () => {
    ...
    describe('2. list command', () => {
        ...
        it('fails with auth error when not authenticated', () => {
            ...
            expect(stderr).toContain('agenthub-agent auth login');
        });
    });
...
describe('Smoke: Error handling', () => {
    ...
    describe('server error handling (via API error mapper)', () => {
        it('HTTP 401 maps to re-authenticate message', async () => {
            ...
            const errorMsg = 'Authentication expired. Run `agenthub-agent auth login` to re-authenticate.';
            expect(errorMsg).toContain('agenthub-agent auth login');
        });
...
describe('Smoke: Interop — dataKey vs legacy encryption', () => {
    ...
    it('session created with legacy encryption (agenthub-cli style) can be decrypted', () => {
        const creds = makeCredentials();
    ...
    it('messages encrypted with dataKey can be round-tripped', () => {
        const sessionKey = getRandomBytes(32);
        const messageContent = { role: 'user', content: { type: 'text', text: 'Hello from agenthub-agent' } };
...
describe('Smoke: Output formatting', () => {
        ...
        expect(result).toContain('No AgentHub sessions yet');
        expect(result).not.toContain('Items: none');
    });
    ...
    it('formatMessageHistory handles empty messages', () => {
        ...
        expect(result).toContain('No AgentHub messages yet');
        expect(result).not.toContain('Items: none');
    });
...
describe('Smoke: Full test suite runs', () => {
    ...
    it('config loads with correct defaults', () => {
        const origUrl = process.env.AGENTHUB_SERVER_URL;
        const origHome = process.env.AGENTHUB_HOME_DIR;
        ...
        try {
            ...
            expect(config.homeDir).toContain('.agenthub');
            expect(config.credentialPath).toContain('agent.key');
        } finally {
            if (origUrl !== undefined) process.env.AGENTHUB_SERVER_URL = origUrl;
            if (origHome !== undefined) process.env.AGENTHUB_HOME_DIR = origHome;
        }
    });
});
