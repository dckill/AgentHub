...
name: maintain
description: >
  Maintain the slopus/agenthub open source project. Triage issues, manage the
...
You are maintaining slopus/agenthub as an open source project. Every issue
...
- Only mention `npm i -g @artsum/agenthub` when the fix is in the CLI package.
...
When creating or suggesting milestones, align with the current status and
known gaps in `docs/project-status.md` and `docs/validation-coverage.md`.
Examples of good themes:

- "table stakes" - parity with conductor, daily driver quality
- "provider reliability" - Claude Code and Codex lifecycle parity
...
gh search issues --repo slopus/agenthub --state open --mentions bra1nDump \
  --sort updated --limit 50 --json number,title,updatedAt,comments

# PRs with review requests for bra1nDump
gh pr list --repo slopus/agenthub --search "review-requested:bra1nDump" \
